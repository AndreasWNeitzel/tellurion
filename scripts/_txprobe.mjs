...
for (const [tag, name] of [['c', 'central transit'], ['g', 'grazing transit'], ['n', 'no transit'], ['h', 'hot Jupiter'], ['e', 'Earth analogue']]) {
  await page.evaluate((n) => { const s=document.querySelectorAll('select'); for (const e of s) if ([...e.options].some(o => o.text === n)) { e.value=n; e.dispatchEvent(new Event('change',{bubbles:true})); } }, name);
  await page.waitForTimeout(2500);
  await page.screenshot({ path: `/tmp/tx-${tag}.png` });
