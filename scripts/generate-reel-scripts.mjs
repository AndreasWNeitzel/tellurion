...
function extract(dir) {
  ...
  const intro = introM ? stripTags(introM[1]) : (fm.one_paragraph || '');
  const tryM = html.match(/playground-what-to-try[\s\S]*?<ul>([\s\S]*?)<\/ul>/);
  const tries = tryM ? [...tryM[1].matchAll(/<li[^>]*>([\s\S]*?)<\/li>/g)].map((x) => stripTags(x[1])) : [];
  const capM = html.match(/<figcaption[^>]*>([\s\S]*?)<\/figcaption>/);
  const caption = capM ? stripTags(capM[1]) : (fm.caption || '');
  const source = (caption.match(/(?:Source|Reference)[:\s]+([^.]+\.)/i) || [])[1] || fm.primary_citation || '';
  return { title, intro, tries, caption, source, hook: fm.hook || '' };
}
...
function buildReel(slug, e) {
  ...
  const tryIt = e.tries[0] || 'Drag the controls and watch the whole picture respond.';
  const tryIt2 = e.tries[1] || '';
