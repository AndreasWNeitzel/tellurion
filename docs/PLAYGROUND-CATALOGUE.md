...
Generated by scripts/make-catalogue.mjs. Total 310 playgrounds: 278 shipped (folder has a .verified marker with a verified-at line), 32 Draft (no .verified, so build-index.mjs gates them out of the gallery), 48 post-build-swept (have DEVNOTES.md), 68 flagged hero_candidate.
...
`playgrounds/bsc-y1s1/CC1017-pathfinding-dijkstra-astar` | CC1017 | DRAFT | swept | HERO-CANDIDATE | tier:simple
...
`playgrounds/bsc-y1s1/FIS1013-atwood-machine-constrained` | F1006 | DRAFT | swept | tier:medium
...
`playgrounds/bsc-y1s1/FIS1013-bernoulli-air-blower-ball` | FIS1013 | DRAFT | swept | HERO-CANDIDATE | tier:simple
...
`playgrounds/bsc-y1s1/FIS1013-bouncing-shapes-concave-surface` | FIS1013 | DRAFT | swept | HERO-CANDIDATE | tier:simple
...
`playgrounds/bsc-y1s1/FIS1013-central-force-orbit-gallery` | F1006 | DRAFT | swept | tier:medium
...
`playgrounds/bsc-y1s1/FIS1013-collision-scattering-lab` | F1006 | DRAFT | swept | tier:medium
...
`playgrounds/bsc-y1s1/FIS1013-projectile-drag-magnus-3d` | F1006 | DRAFT | swept | tier:medium

- What it does / physics / visuals: A ball launched in 3D under gravity, quadratic drag and the spin-dependent Magnus force, integrated with RK4. Three trajectories fly at once over a perspective ground grid: vacuum, drag only, and drag plus Magnus. Because the Magnus force is perpendicular to both velocity and spin, sidespin curves the flight laterally while backspin adds lift and topspin cuts the range.
...
`playgrounds/bsc-y1s1/FIS1013-rigid-body-euler-3d` | F1006 | DRAFT | swept | HERO-CANDIDATE | tier:hero
...
`playgrounds/bsc-y1s1/FIS1013-tennis-racket-theorem` | FIS1013 | DRAFT | swept | HERO-CANDIDATE | tier:simple
...
`playgrounds/bsc-y1s2/FIS1014-biot-savart-3d-explorer` | FIS1004 | DRAFT | swept | HERO-CANDIDATE | tier:advanced

- What it does / physics / visuals: The magnetic field of arbitrary current loops computed directly from the Biot-Savart law and shown as a 3D lattice of arrow glyphs coloured by |B| with traced field lines and an on-axis Bz profile. Presets give the straight wire, circular loop, Helmholtz coils and a solenoid; current and coil radius are sliders; drag to orbit. The headless sim.js is gate-tested against the analytic closed forms.
...
### Gauss Law in 2D
`playgrounds/bsc-y1s2/FIS1014-gauss-law-flux-through-surface` | FIS1014 | SHIPPED | tier:simple

- What it does / physics / visuals: A 2D point charge $q$ at user-set position generates the planar Coulomb field $\mathbf{E} = q / (2 \pi \epsilon_0 r) \hat r$. A user-controlled closed curve (ellipse or perturbed-ellipse blob) is drawn around the charge, and the flux $\oint \mathbf{E} \cdot \hat n\,ds$ is computed numerically via Simpson quadrature.
- Hook: (placeholder hook, flagged)
- Review verdict: [deep] RENDER-NEUTRAL TEXT FIX ONLY

### Interactive Laplace Solver: Draw Your Own Conductors
`playgrounds/bsc-y1s2/FIS1014-laplace-solver-2d-interactive` | FIS1004 | DRAFT | HERO-CANDIDATE | tier:advanced
...
`playgrounds/bsc-y1s2/FIS1014-magnetic-hysteresis-bh-curve` | FIS1004 | DRAFT | tier:medium
...
`playgrounds/bsc-y1s2/FIS1014-method-of-images-2d` | FIS1014 | SHIPPED | tier:simple

- What it does / physics / visuals: A point charge above a grounded conducting plane: the field is built by adding the image charge below the plane. The induced surface charge on the conductor integrates to negative the real charge. Source: Griffiths E&M Ch. 3.2 (`griffiths-em`).
- Hook: (placeholder hook, flagged)
- Review verdict: [deep] RENDER-NEUTRAL TEXT FIX ONLY

### Multipole Expansion: Exact vs Truncated Potential
`playgrounds/bsc-y1s2/FIS1014-multipole-expansion-3d` | FIS1004 | DRAFT | tier:medium
...
`playgrounds/bsc-y1s2/FIS1014-poynting-vector-wave-3d` | FIS1004 | DRAFT | HERO-CANDIDATE | tier:advanced
...
`playgrounds/bsc-y1s2/FIS1015-michelson-fringe-counter` | FIS1015 | SHIPPED | tier:simple

- What it does / physics / visuals: Moving one arm of a Michelson interferometer by $\lambda/2$ produces one full fringe at the center. The rendered pattern is the ring system for a divergent source. Source: Hecht Optics Ch. 9.4 (`hecht2017`).
- Hook: (placeholder hook, flagged)
- Review verdict: [deep] RENDER-NEUTRAL TEXT FIX ONLY

### Line Integral vs Path
`playgrounds/bsc-y1s2/M1015-line-integral-vs-path` | M1015 | SHIPPED | tier:simple
...
`playgrounds/bsc-y1s2/M1015-multiple-integral-fubini` | M1015 | SHIPPED | tier:simple

- What it does / physics / visuals: Iterated integrals over a rectangle in two orders match numerically. Demonstrated on $f(x, y) = \sin x \cos y$. Source: Riley-Hobson Ch. 10 (`riley-hobson`).
- Hook: (placeholder hook, flagged)
- Review verdict: [deep] RENDER-NEUTRAL TEXT FIX ONLY
...
`playgrounds/bsc-y2s1/AST2004-bohr-hydrogen-spectrum` | AST2004 | SHIPPED | tier:simple

- What it does / physics / visuals: A single electron orbits a proton in a Coulomb potential. The Bohr quantization condition $L = n \hbar$ pins discrete orbits indexed by integer $n \ge 1$ with energies $E_n = -E_R / n^2$, where $E_R = 13.605693$ eV is the (infinite-mass) Rydberg energy. Transitions between levels emit photons of wavelength $$\frac{1}{\lambda} = R_H \left( \frac{1}{n_\ell^2} - \frac{1}{n_h^2} \right)$$ with $R_H \approx 1.09678 \times 10^7$ m$^{-1}$ the hydrogen Rydberg constant (proton-mass corrected).
- Hook: (placeholder hook, flagged)
- Review verdict: [deep] RENDER-NEUTRAL TEXT FIX ONLY

### Jeans Instability
`playgrounds/bsc-y2s1/AST2004-jeans-instability` | AST2004 | SHIPPED | tier:simple

- What it does / physics / visuals: A uniform, self-gravitating, isothermal hydrogen medium with mass density $\rho$ and sound speed $c_s = \sqrt{k_B T / m_p}$. Linear perturbations $\propto \exp(i k x - i \omega t)$ satisfy the dispersion relation
- Hook: (placeholder hook, flagged)
- Review verdict: [deep] RENDER-NEUTRAL TEXT FIX ONLY

### Keplerian Orbit Elements
`playgrounds/bsc-y2s1/AST2004-kepler-orbit-elements` | AST2004 | SHIPPED | tier:simple

- What it does / physics / visuals: Vary the six classical elements (a, e, i, Ω, ω, ν) and watch a 3D orbit redraw. Source: Carroll-Ostlie Ch. 2 (`carroll-ostlie`).
- Hook: (placeholder hook, flagged)
- Review verdict: [deep] RENDER-NEUTRAL TEXT FIX ONLY
- Flags to address:
...
`playgrounds/bsc-y2s1/AST2004-kepler-orbit-explorer` | AST2004 | SHIPPED | tier:simple
...
`playgrounds/bsc-y2s1/AST2004-lagrange-points-cr3bp` | AST2004 | SHIPPED | tier:simple

- What it does / physics / visuals: Two heavy bodies of mass m1 and m2 orbit their common center of mass in a circular orbit. A test particle (mass negligible) moves under their combined gravity, computed in the rotating frame where the two primaries stand still. Non-dimensional units: total mass = 1, separation = 1, angular velocity = 1.
- Hook: (placeholder hook, flagged)
- Review verdict: [deep] RENDER-NEUTRAL TEXT FIX ONLY

### Perihelion Precession in a Schwarzschild Effective Potential
`playgrounds/bsc-y2s1/AST2004-mercury-precession-pn` | AST2004 | SHIPPED | tier:simple

- What it does / physics / visuals: In pure Newtonian gravity, Bertrand's theorem says that the only closed bound orbits in central potentials are those of V(r) ~ 1/r and V(r) ~ r^2. Any departure from these forms causes the orbit to fail to close: the perihelion moves around with each revolution. The 1PN correction in the orbit-averaged Schwarzschild metric introduces an effective extra 1/r^3 term that breaks the 1/r form and produces a perihelion advance.
- Hook: (placeholder hook, flagged)
- Review verdict: [deep] RENDER-NEUTRAL TEXT FIX ONLY

### Radial Velocity Curve from Orbital Elements
`playgrounds/bsc-y2s1/AST2004-radial-velocity-orbital-trace` | AST2004 | SHIPPED | tier:simple

- What it does / physics / visuals: Orbit and corresponding RV curve, side by side. Source: Carroll-Ostlie Ch. 7 (`carroll-ostlie`).
- Hook: (placeholder hook, flagged)
- Review verdict: [deep] RENDER-NEUTRAL TEXT FIX ONLY

### Tidal Disruption Near a Massive Primary
`playgrounds/bsc-y2s1/AST2004-roche-tidal-disruption` | AST2004 | SHIPPED | tier:simple

- What it does / physics / visuals: A cloud of 80 self-gravitating test particles ("a fluid satellite") on an eccentric orbit around a heavy point-mass primary. When the orbit takes the cloud inside the Roche radius, the tidal force from the primary overwhelms the satellite's self-gravity and stretches it into a stream. Outside the Roche radius the cloud holds together.
- Hook: (placeholder hook, flagged)
- Review verdict: [deep] RENDER-NEUTRAL TEXT FIX ONLY

### Rotation Curve Explorer
`playgrounds/bsc-y2s1/AST2004-rotation-curve-explorer` | AST2004 | SHIPPED | tier:simple

- What it does / physics / visuals: A face-on synthetic spiral galaxy with a Hernquist bulge ($M_b = 10^{10} M_\odot$, $a_b = 0.5$ kpc) and a Miyamoto-Nagai disk ($M_d = 6 \times 10^{10} M_\odot$, $a_d = 4$ kpc, $b_d = 0.3$ kpc), seen from above. The same visible mass is present in all three models; what changes is the assumption about unseen mass.
- Hook: (placeholder hook, flagged)
- Review verdict: [deep] RENDER-NEUTRAL TEXT FIX ONLY

### Saha-Boltzmann Hydrogen Ionization
`playgrounds/bsc-y2s1/AST2004-saha-boltzmann-ionization` | AST2004 | SHIPPED | tier:simple

- What it does / physics / visuals: Pure-hydrogen plasma in local thermodynamic equilibrium. Charge balance ($n_e = n_+$) and total nucleon conservation ($n_\text{tot} = n_+ + n_0$) plus the Saha equation give a closed-form quadratic for the ionization fraction $x = n_+ / n_\text{tot}$:
- Hook: (placeholder hook, flagged)
- Review verdict: [deep] RENDER-NEUTRAL TEXT FIX ONLY

### Schwarzschild Effective Potential and the ISCO
`playgrounds/bsc-y2s1/AST2004-schwarzschild-effective-potential` | AST2004 | SHIPPED | tier:simple

- What it does / physics / visuals: Effective radial potential for geodesics outside a Schwarzschild black hole of mass M (geometric units G = c = 1): - Massive: V_eff = 0.5 (1 - 2M/r)(1 + L^2 / r^2) - 0.5. - Photon: V_eff = 0.5 (L^2 / r^2)(1 - 2M/r).
- Hook: (placeholder hook, flagged)
- Review verdict: [deep] RENDER-NEUTRAL TEXT FIX ONLY

### Stellar Blackbody + Absorption Lines
`playgrounds/bsc-y2s1/AST2004-stellar-blackbody-vs-line` | AST2004 | SHIPPED | tier:simple

- What it does / physics / visuals: Planck continuum plus Balmer / Ca II / Na D absorption lines. Source: Carroll-Ostlie Ch. 3 (`carroll-ostlie`).
- Hook: (placeholder hook, flagged)
- Review verdict: [deep] RENDER-NEUTRAL TEXT FIX ONLY

### Three-Body Figure-Eight Choreography
`playgrounds/bsc-y2s1/AST2004-three-body-orbit` | AST2004 | SHIPPED | tier:simple

- What it does / physics / visuals: Three equal masses $m_1 = m_2 = m_3 = 1$ interact under Newtonian gravity in 2D with $G = 1$. At the Chenciner-Montgomery initial condition (2000) the three masses chase one another on a single closed figure-eight curve, with period $T \approx 6.3259$. This is the most famous "choreography" solution of the planar three-body problem. The playground integrates the system with the symplectic engine and renders the three trails. A slider perturbs the initial velocity of body 3 by a small amount $\delta v$; at $\delta v = 0$ the choreography is stable indefinitely, and at $\delta v > 0$ the system slowly destabilizes.
- Hook: (placeholder hook, flagged)
- Review verdict: [deep] RENDER-NEUTRAL TEXT FIX ONLY

### Mandel-Agol Analytic Transit
`playgrounds/bsc-y2s1/AST2004-transit-mandel-agol-analytic` | AST2004 | SHIPPED | tier:simple

- What it does / physics / visuals: Analytic transit light curve; uniform-source closed form plus quadratic limb darkening via ring decomposition. Source: Mandel & Agol 2002 (`mandelagol2002`).
- Hook: (placeholder hook, flagged)
- Review verdict: [deep] RENDER-NEUTRAL TEXT FIX ONLY

### Elastic Waves: P and S Modes in a Solid
`playgrounds/bsc-y2s1/FIS2002-elastic-wave-modes-solid` | FIS2002 | DRAFT | HERO-CANDIDATE | tier:advanced

- What it does / physics / visuals: A 2D isotropic elastic solid solved by an explicit leapfrog scheme for the vector displacement. A source launches a compressional P wave at v_P = sqrt((lambda+2mu)/rho) and a shear S wave at v_S = sqrt(mu/rho). The primary scene is the physical medium: the divergence (compression) coloured, a reference grid straining, and the analytic P and S wavefront rings; the side panel is the seismogram at a 45-degree station where the P arrival precedes the S arrival by d (1/v_S - 1/v_P). Lowering the shear modulus toward zero removes the S wave entirely. The headless sim.js is gate-tested for the analytic speeds, P faster than S, the measured front speeds, the vanishing shear front as mu to 0, the seismograph delay, and CFL stability.
...
`playgrounds/bsc-y2s1/FIS2002-huygens-construction-interactive` | FIS2002 | DRAFT | tier:advanced
...
`playgrounds/bsc-y2s1/FIS2002-normal-modes-nd-chain` | FIS2002 | DRAFT | HERO-CANDIDATE | tier:advanced
...
`playgrounds/bsc-y2s1/FIS2002-wave-2d-complex-geometry` | FIS2002 | SHIPPED | HERO-CANDIDATE | tier:advanced
...
`playgrounds/bsc-y2s1/FIS2013-cyclotron-uniform-b` | FIS2013 | SHIPPED | tier:simple

- What it does / physics / visuals: A charged particle (q = m = 1) in a uniform, out-of-page magnetic field B = B z-hat. Initial state: (x, y) = (0, 0), (vx, vy) = (0, v).
- Hook: (placeholder hook, flagged)
- Review verdict: [deep] RENDER-NEUTRAL TEXT FIX ONLY

### E x B Drift and the Cycloid
`playgrounds/bsc-y2s1/FIS2013-exb-drift-cycloid` | FIS2013 | SHIPPED | tier:simple

- What it does / physics / visuals: A charged particle (q = m = 1) in crossed uniform fields B = B z-hat (out of page) and E = E x-hat. Starting from rest at the origin, the particle follows a cycloid: it accelerates in +x under E until v x B curves it back. The net motion is a uniform drift in (E x B) / B^2 = -E / B in y.
- Hook: (placeholder hook, flagged)
- Review verdict: [deep] RENDER-NEUTRAL TEXT FIX ONLY

### Larmor Radiation Pattern
`playgrounds/bsc-y2s1/FIS2013-larmor-radiation-pattern` | FIS2013 | SHIPPED | tier:simple

- What it does / physics / visuals: Non-relativistic accelerating charge radiates with the $\sin^2\theta$ angular distribution; total power follows the Larmor formula. Source: Griffiths E&M Ch. 11 (`griffiths-em`).
- Hook: (placeholder hook, flagged)
- Review verdict: [deep] RENDER-NEUTRAL TEXT FIX ONLY

### Lienard-Wiechert Beaming and Synchrotron Lobe
`playgrounds/bsc-y2s1/FIS2013-lienard-wiechert-synchrotron` | FIS2013 | SHIPPED | tier:simple

- What it does / physics / visuals: Relativistic radiation collimates into a forward cone of opening angle $\sim 1/\gamma$. Two limits (a parallel and a perpendicular to v) are shown. Source: Jackson 3e Ch. 14 (`jackson3e`).
- Hook: (placeholder hook, flagged)
- Review verdict: [deep] RENDER-NEUTRAL TEXT FIX ONLY

### Skin Effect in a Conductor
`playgrounds/bsc-y2s1/FIS2013-skin-effect-1d-conductor` | FIS2013 | SHIPPED | tier:simple

- What it does / physics / visuals: Exponential decay of the AC electric field inside a conductor; skin depth $\delta = \sqrt{2/(\omega \mu \sigma)}$. Source: Griffiths E&M Ch. 9 (`griffiths-em`).
- Hook: (placeholder hook, flagged)
- Review verdict: [deep] RENDER-NEUTRAL TEXT FIX ONLY

### Transmission Line Impedance Matching
`playgrounds/bsc-y2s1/FIS2013-transmission-line-impedance-matching` | FIS2013 | SHIPPED | tier:simple

- What it does / physics / visuals: A coaxial transmission line of characteristic impedance $Z_0 = 50\,\Omega$ terminated by a resistive load $Z_L$. The forward and reflected voltage waves superpose into a standing pattern whose amplitude envelope depends on the mismatch.
- Hook: (placeholder hook, flagged)
- Review verdict: [deep] RENDER-NEUTRAL TEXT FIX ONLY

### BTW Sandpile and Self-Organized Criticality
`playgrounds/bsc-y2s1/FIS2014-abelian-sandpile-btw` | FIS2014 | SHIPPED | tier:simple

- What it does / physics / visuals: 32 x 32 lattice of integer heights. Drop a grain at a random site; topple when height >= 4. Boundary sites lose grains to the outside. After enough drops the system settles into a critical state where avalanche-size distribution is a power law P(s) ~ s^(-tau), tau ~ 1.21 in 2D.
- Hook: (placeholder hook, flagged)
- Review verdict: [deep] RENDER-NEUTRAL TEXT FIX ONLY

### Adiabatic vs Isothermal Processes on a PV Diagram
`playgrounds/bsc-y2s1/FIS2014-adiabatic-vs-isothermal-pv` | FIS2014 | SHIPPED | tier:simple

- What it does / physics / visuals: Same initial state, two reversible processes; the adiabatic is steeper. Source: Callen Ch. 4 (`callen`).
- Hook: (placeholder hook, flagged)
- Review verdict: [deep] NEEDS CODE FIX + RECAPTURE

### Brownian Motion and the Diffusion Law
`playgrounds/bsc-y2s1/FIS2014-brownian-motion-diffusion` | FIS2001 | DRAFT | HERO-CANDIDATE | tier:advanced

- What it does / physics / visuals: An ensemble of 1600 independent two-dimensional walkers diffuses from the origin, each axis taking Gaussian steps, so the cloud spreads as a Gaussian whose mean-squared displacement is 4Dt and a highlighted tracer drags a random-walk trail under solvent buffeting. The diffusion coefficient is the Stokes-Einstein value D = kB T / (6 pi eta r): heating, thinning the solvent, or shrinking the tracer all visibly speed the spread. Side panels track the measured mean-squared displacement against the 4Dt line and the displacement histogram against the Gaussian. The headless sim.js is gate-tested for the 4Dt law, time linearity, isotropy, Gaussianity (KS), the Stokes-Einstein scaling, and seed determinism.
...
`playgrounds/bsc-y2s1/FIS2014-engine-cycle-explorer` | FIS2014 | SHIPPED | tier:simple

- What it does / physics / visuals: Four idealized cycles on the PV plane: Otto, Diesel, Carnot, Stirling. Source: Callen Ch. 4-5 (`callen`).
- Hook: (placeholder hook, flagged)
- Review verdict: [deep] RENDER-NEUTRAL TEXT FIX ONLY
- Flags to address:
...
`playgrounds/bsc-y2s1/FIS2014-equipartition-from-collisions` | FIS2014 | SHIPPED | tier:simple

- What it does / physics / visuals: 2D hard-disk gas; the mean translational kinetic energy converges to $kT$. Source: Reif Ch. 7 (`reif`).
- Hook: (placeholder hook, flagged)
...
`playgrounds/bsc-y2s1/FIS2014-maxwell-boltzmann-emergence` | FIS2014 | SHIPPED | tier:simple

- What it does / physics / visuals: A 2D box of side L = 8 containing N = 80 hard disks (radius 0.15) initially moving with identical speed v_0 = 1 but random orientations. Walls are reflecting; disk-disk collisions are elastic with equal masses (which exchanges only the velocity components along the contact normal).
- Hook: (placeholder hook, flagged)
- Review verdict: [deep] RENDER-NEUTRAL TEXT FIX ONLY
- Flags to address:
  - `hook: 'STATUS: needs_hook'` — must be replaced with concrete 1-sentence hook
...
`playgrounds/bsc-y2s1/FIS2016-airy-pattern-circular-aperture` | FIS2016 | SHIPPED | tier:simple

- What it does / physics / visuals: The Fraunhofer far-field intensity from a uniformly illuminated circular aperture of radius a. This is the classical resolution-limiting pattern for any optical instrument with a round pupil: telescopes, microscopes, eyes.
- Hook: (placeholder hook, flagged)
- Review verdict: [deep] RENDER-NEUTRAL TEXT FIX ONLY
- Flags to address:
  - `hook: 'STATUS: needs_hook'` — must be replaced with concrete 1-sentence hook
...
`playgrounds/bsc-y2s1/FIS2016-group-vs-phase-velocity` | FIS2016 | SHIPPED | tier:simple

- What it does / physics / visuals: Two-component superposition; envelope and carrier move at different speeds in dispersive media. Source: Crawford Ch. 6 (`crawford-waves`).
- Hook: (placeholder hook, flagged)
- Review verdict: [deep] RENDER-NEUTRAL TEXT FIX ONLY
- Flags to address:
  - `hook: 'STATUS: needs_hook'` — must be replaced with concrete 1-sentence hook
...
`playgrounds/bsc-y2s1/FIS2016-single-double-multi-slit` | FIS2016 | SHIPPED | tier:simple

- What it does / physics / visuals: N identical slits of width a and center-to-center separation d, illuminated by collimated coherent light of wavelength lambda at normal incidence. Far-field intensity at angle theta:
- Hook: (placeholder hook, flagged)
- Review verdict: [deep] RENDER-NEUTRAL TEXT FIX ONLY
- Flags to address:
  - `hook: 'STATUS: needs_hook'` — must be replaced with concrete 1-sentence hook
...
`playgrounds/bsc-y2s1/FIS2016-standing-waves-string-modes` | FIS2016 | SHIPPED | tier:simple

- What it does / physics / visuals: A uniform string of length L is fixed at both ends. The normal modes of small transverse oscillation are y_n(x, t) = sin(n pi x / L) cos(2 pi f_n t), f_n = n c / (2 L).
- Hook: (placeholder hook, flagged)
- Review verdict: [deep] RENDER-NEUTRAL TEXT FIX ONLY
- Flags to address:
  - `hook: 'STATUS: needs_hook'` — must be replaced with concrete 1-sentence hook
...
`playgrounds/bsc-y2s1/FIS2016-transverse-vs-longitudinal-mode` | FIS2016 | SHIPPED | tier:simple

- What it does / physics / visuals: Same dispersion, different polarization. Source: Crawford Ch. 5 (`crawford-waves`).
- Hook: (placeholder hook, flagged)
- Review verdict: [deep] RENDER-NEUTRAL TEXT FIX ONLY
- Flags to address:
  - `hook: 'STATUS: needs_hook'` — must be replaced with concrete 1-sentence hook
...
`playgrounds/bsc-y2s1/FIS2016-wave-on-string-reflection` | FIS2016 | SHIPPED | tier:simple

- What it does / physics / visuals: Two parallel 1D strings of length L = 4 with c = 1, each with a Gaussian pulse launched moving rightward. Top string: fixed ends (y = 0 at both boundaries). Bottom string: free ends (y_x = 0 boundary, i.e. the neighbor mirrors at the boundary). Pulse reflects when it reaches each boundary.
- Hook: (placeholder hook, flagged)
- Review verdict: [deep] RENDER-NEUTRAL TEXT FIX ONLY
- Flags to address:
  - `hook: 'STATUS: needs_hook'` — must be replaced with concrete 1-sentence hook
...
`playgrounds/bsc-y2s1/FIS2016-wavepacket-dispersion-1d` | FIS2016 | SHIPPED | tier:simple

- What it does / physics / visuals: The packet center drifts at $\hbar k_0 / m$; the width broadens as $\sigma(t) = \sigma_0\sqrt{1 + (\hbar t/2m\sigma_0^2)^2}$. Source: Eisberg-Resnick Ch. 5 (`eisberg-resnick`).
- Hook: (placeholder hook, flagged)
- Review verdict: [deep] RENDER-NEUTRAL TEXT FIX ONLY
- Flags to address:
  - `hook: 'STATUS: needs_hook'` — must be replaced with concrete 1-sentence hook
...
`playgrounds/bsc-y2s1/M2037-stokes-theorem-2d-circulation` | M2037 | SHIPPED | tier:simple

- What it does / physics / visuals: Three vector fields (unit-curl, shear, conservative) and a draggable rectangle. Circulation = $\iint$ curl $dA$ closed-form for these uniform-curl fields. Source: Riley-Hobson Ch. 10 (`riley-hobson`).
- Hook: (placeholder hook, flagged)
- Review verdict: [deep] RENDER-NEUTRAL TEXT FIX ONLY
- Flags to address:
  - `hook: 'STATUS: needs_hook'` — must be replaced with concrete 1-sentence hook
...
`playgrounds/bsc-y2s1/M2037-vector-field-div-curl-visualizer` | M2037 | SHIPPED | tier:simple

- What it does / physics / visuals: A parameterized 2D vector field $\mathbf{F}(x, y; a)$ visualized as a grid of arrows on a $\pm 3 \times \pm 2$ region. Four families:
- Hook: (placeholder hook, flagged)
- Review verdict: [deep] RENDER-NEUTRAL TEXT FIX ONLY
- Flags to address:
  - `hook: 'STATUS: needs_hook'` — must be replaced with concrete 1-sentence hook
...
`playgrounds/bsc-y2s2/FIS2006-dipole-radiation-3d` | FIS2006 | DRAFT | HERO-CANDIDATE | tier:hero
...
`playgrounds/bsc-y2s2/FIS2017-compton-scattering-kinematics` | FIS2017 | SHIPPED | tier:simple

- What it does / physics / visuals: A monochromatic photon of wavelength $\lambda$ (typically 0.5 to 10 pm, the X-ray regime where Compton scattering is significant) is incident along the $+x$ axis on a free electron at rest. The photon scatters at angle $\theta$ measured from its original direction. The recoiling electron flies off at angle $\phi$ on the opposite side of the scattering plane.
- Hook: (placeholder hook, flagged)
...
`playgrounds/bsc-y2s2/FIS2017-davisson-germer-diffraction` | FIS2017 | SHIPPED | tier:simple

- What it does / physics / visuals: Electrons accelerated through a voltage $V$ are scattered off the (111) face of a nickel crystal. The atomic row spacing on that surface is $D = 0.215$ nm. Constructive interference appears at angle $\theta$ from the normal satisfying
- Hook: (placeholder hook, flagged)
- Review verdict: [first-pass] RENDER-NEUTRAL TEXT FIX ONLY
- Flags to address:
  - [high] spec.md placeholder on lines 12-13: `hook: 'STATUS: needs_hook'` and `one_paragraph: 'STATUS: needs_paragraph'` -> Write hook and paragraph
...
`playgrounds/bsc-y2s2/FIS2017-de-broglie-wavelength` | FIS2017 | SHIPPED | tier:simple
...
`playgrounds/bsc-y2s2/FIS2017-photoelectric-effect-threshold` | FIS2017 | SHIPPED | tier:simple

- What it does / physics / visuals: Monochromatic light of frequency $\nu$ illuminates a clean metal cathode with work function $\phi$. Einstein 1905: each photon delivers exactly $h\nu$ of energy to a bound electron. Electrons appear only if $h\nu \ge \phi$. The maximum kinetic energy of the ejected electron is $$KE_{max} = h\nu - \phi, \qquad \nu > \nu_0 \equiv \phi / h.$$ Below threshold, no electrons are ejected, regardless of light intensity. The threshold is a sharp cutoff in frequency, not in intensity, which is the central quantum signature.
- Hook: (placeholder hook, flagged)
- Review verdict: [deep] RENDER-NEUTRAL TEXT FIX ONLY
- Flags to address:
...
`playgrounds/bsc-y2s2/FIS2018-advection-scheme-shootout` | FIS2018 | SHIPPED | tier:simple

- What it does / physics / visuals: 1D linear advection u_t + c u_x = 0 on a periodic domain [0, 1] with a square pulse initial condition. Four numerical schemes solve the same problem side-by-side; the dashed green line is the analytic solution (pure translation of the pulse).
- Hook: (placeholder hook, flagged)
- Review verdict: [deep] CLEAN (deep audit passed)
- Flags to address:
...
`playgrounds/bsc-y2s2/FIS2018-gauss-quadrature-vs-trapezoid` | FIS2018 | SHIPPED | tier:simple

- What it does / physics / visuals: Numerical evaluation of integral_{-1}^1 f(x) dx by: - Trapezoidal rule: n + 1 equispaced points. - Gauss-Legendre: n optimized nodes from roots of P_n.
- Hook: (placeholder hook, flagged)
- Review verdict: [deep] RENDER-NEUTRAL TEXT FIX ONLY
- Flags to address:
...
`playgrounds/bsc-y2s2/FIS2018-linear-system-direct-vs-iterative` | FIS2018 | SHIPPED | tier:simple

- What it does / physics / visuals: Poisson 1D problem; Thomas tridiagonal direct solver vs Jacobi, Gauss-Seidel, CG. Source: Villate Ch. 6 (`villate-vpython`).
- Hook: (placeholder hook, flagged)
- Review verdict: [deep] RENDER-NEUTRAL TEXT FIX ONLY
- Flags to address:
...
`playgrounds/bsc-y2s2/FIS2018-mc-integration-convergence` | FIS2018 | SHIPPED | tier:simple

- What it does / physics / visuals: Estimate integral_{0}^{1} f(x) dx with f(x) = 1 + 10 (x - 1/2)^4. Exact value: 1.125. Two estimators: - Plain: I_hat = (1/N) sum f(U_i), U_i ~ U(0, 1). - Importance: I_hat = (1/N) sum f(X_i) / q(X_i), X_i ~ Beta(2, 2).
- Hook: (placeholder hook, flagged)
...
`playgrounds/bsc-y2s2/FIS2018-ode-solver-euler-rk4-rk45` | FIS2018 | SHIPPED | tier:simple

- What it does / physics / visuals: Three integrators on the simple harmonic oscillator; Euler drifts energy upward, RK4 is accurate, RK45 estimates its own error. Source: Villate VPython Numerical Methods (`villate-vpython`).
- Hook: (placeholder hook, flagged)
- Review verdict: [deep] RENDER-NEUTRAL TEXT FIX ONLY
- Flags to address:
  - Figcaption or user-visible text exposes raw bibliography key in backticks: `villate-vpython`. This is a rendering artifact and must be removed. Change all instances of `(villate-vpython)` or text cont
...
`playgrounds/bsc-y2s2/FIS2018-root-finding-bisect-newton-secant` | FIS2018 | SHIPPED | tier:simple

- What it does / physics / visuals: Bisection, Newton-Raphson, and the secant method on a selectable test function. Source: Villate VPython Numerical Methods Ch. 4 (`villate-vpython`).
- Hook: (placeholder hook, flagged)
- Review verdict: [deep] RENDER-NEUTRAL TEXT FIX ONLY
- Flags to address:
...
`playgrounds/bsc-y2s2/FIS2018-runge-vs-chebyshev` | FIS2018 | SHIPPED | tier:simple

- What it does / physics / visuals: Polynomial interpolation of the Runge function f(x) = 1 / (1 + 25 x^2) on [-1, 1] at n + 1 nodes: - Equispaced: x_i = -1 + 2 i / n. - Chebyshev (second kind, including endpoints): x_i = cos(i pi / n).
- Hook: (placeholder hook, flagged)
- Review verdict: [deep] RENDER-NEUTRAL TEXT FIX ONLY
- Flags to address:
  - Figcaption or user-visible text exposes raw bibliography key in backticks: `trefethen-spectral`. This is a rendering artifact and must be removed. Change all instances of `(trefethen-spectral)` or tex
...
`playgrounds/bsc-y2s2/FIS2021-billiards-circle-stadium-sinai` | FIS2021 | SHIPPED | tier:simple

- What it does / physics / visuals: A free particle of unit speed bouncing elastically off the walls of a 2D shape. Three classical geometries: circle (integrable), Bunimovich stadium (chaotic), Sinai billiard (chaotic with convex scatterer). Used to study quantum-classical correspondence and the onset of chaos under purely geometric constraints.
- Hook: (placeholder hook, flagged)
- Review verdict: [deep] RENDER-NEUTRAL TEXT FIX ONLY
- Flags to address:
...
`playgrounds/bsc-y2s2/FIS2021-canonical-transformation-visual` | FIS2021 | SHIPPED | HERO-CANDIDATE | tier:advanced
...
`playgrounds/bsc-y2s2/FIS2021-coupled-kuramoto-oscillators` | FIS2021 | SHIPPED | tier:simple

- What it does / physics / visuals: N = 128 phase oscillators with intrinsic frequencies omega_i drawn from a Lorentzian distribution with half-width-at-half-maximum gamma. Each oscillator couples globally to the mean phase with strength K: theta_i' = omega_i + (K / N) sum_j sin(theta_j - theta_i)
- Hook: (placeholder hook, flagged)
- Review verdict: [deep] RENDER-NEUTRAL TEXT FIX ONLY
- Flags to address:
...
`playgrounds/bsc-y2s2/FIS2021-duffing-oscillator` | FIS2021 | SHIPPED | tier:simple

- What it does / physics / visuals: A particle in a symmetric double-well potential V(x) = -x^2/2 + x^4/4, subject to linear damping and a periodic external drive. The Duffing equation is the textbook system that exhibits a complete period-doubling cascade to chaos under a single control parameter (the drive amplitude gamma). It is also one of the cleanest examples for visualizing a Poincare stroboscopic section.
- Hook: (placeholder hook, flagged)
- Review verdict: [first-pass] CLEAN (spot audit passed; no exposed bibkeys, files complete, invariants present)

### FitzHugh-Nagumo Excitable Neuron
`playgrounds/bsc-y2s2/FIS2021-fitzhugh-nagumo-excitable` | FIS2021 | SHIPPED | tier:simple

- What it does / physics / visuals: Two-variable reduction of the Hodgkin-Huxley model: v' = v - v^3 / 3 - w + I w' = epsilon (v + a - b w) with a = 0.7, b = 0.8, epsilon = 0.08. v is fast (voltage); w is slow (recovery). External input I tunes the system from excitable rest to sustained limit-cycle firing through a Hopf bifurcation.
- Hook: (placeholder hook, flagged)
- Review verdict: [deep] RENDER-NEUTRAL TEXT FIX ONLY
- Flags to address:
...
`playgrounds/bsc-y2s2/FIS2021-hamilton-jacobi-action-angle` | FIS2021 | SHIPPED | HERO-CANDIDATE | tier:advanced
...
`playgrounds/bsc-y2s2/FIS2021-hamiltonian-phase-space-flow` | FIS2021 | SHIPPED | tier:simple

- What it does / physics / visuals: Click to seed tracers; each one traces an orbit at constant energy. Source: Lemos Ch. 6 (`lemos-mech`).
- Hook: (placeholder hook, flagged)
- Review verdict: [deep] RENDER-NEUTRAL TEXT FIX ONLY
- Flags to address:
...
`playgrounds/bsc-y2s2/FIS2021-henon-strange-attractor` | FIS2021 | SHIPPED | tier:simple
...
`playgrounds/bsc-y2s2/FIS2021-kam-theory-poincare-section` | FIS2021 | SHIPPED | HERO-CANDIDATE | tier:advanced
...
`playgrounds/bsc-y2s2/FIS2021-lagrangian-vs-newtonian` | FIS2021 | SHIPPED | tier:simple

- What it does / physics / visuals: Same planar-pendulum dynamics shown three ways. Source: Lemos Ch. 2-3 (`lemos-mech`).
- Hook: (placeholder hook, flagged)
- Review verdict: [deep] RENDER-NEUTRAL TEXT FIX ONLY
- Flags to address:
  - Figcaption or user-visible text exposes raw bibliography key in backticks: `lemos-mech`. This is a rendering artifact and must be removed. Change all instances of `(lemos-mech)` or text containing bac
...
`playgrounds/bsc-y2s2/FIS2021-lorenz-attractor` | FIS2021 | SHIPPED | tier:simple
...
`playgrounds/bsc-y2s2/FIS2021-lyapunov-spectrum` | FIS2021 | SHIPPED | tier:simple
...
`playgrounds/bsc-y2s2/FIS2021-mandelbrot-explorer` | FIS2021 | SHIPPED | tier:simple

- What it does / physics / visuals: The Mandelbrot set $\mathcal{M} \subset \mathbb{C}$ is
- Hook: (placeholder hook, flagged)
- Review verdict: [first-pass] CLEAN (spot audit passed; no exposed bibkeys, files complete, invariants present)

### Noether's Theorem: Symmetry to Conservation
`playgrounds/bsc-y2s2/FIS2021-noether-symmetry-to-conservation` | FIS2021 | SHIPPED | tier:simple

- What it does / physics / visuals: Rotation symmetry of a central potential preserves $L_z$; breaking the symmetry makes $L_z$ drift. Source: Lemos Ch. 4 (`lemos-mech`).
- Hook: (placeholder hook, flagged)
- Review verdict: [deep] RENDER-NEUTRAL TEXT FIX ONLY
- Flags to address:
  - Figcaption or user-visible text exposes raw bibliography key in backticks: `lemos-mech`. This is a rendering artifact and must be removed. Change all instances of `(lemos-mech)` or text containing bac
...
`playgrounds/bsc-y2s2/FIS2021-predator-prey-hopf` | FIS2021 | SHIPPED | tier:simple

- What it does / physics / visuals: Rosenzweig-MacArthur predator-prey model with Holling Type II response: x' = r x (1 - x / K) - a x y / (b + x) y' = e a x y / (b + x) - d y
- Hook: (placeholder hook, flagged)
- Review verdict: [first-pass] CLEAN (spot audit passed; no exposed bibkeys, files complete, invariants present)

### Rossler Funnel Attractor
`playgrounds/bsc-y2s2/FIS2021-rossler-funnel` | FIS2021 | SHIPPED | tier:simple

- What it does / physics / visuals: Otto Rossler's 1976 minimal continuous-time chaotic system. Three coupled first-order ODEs in (x, y, z) with one quadratic nonlinearity (z times x). Compared to Lorenz, the geometry is simpler: a near-planar spiral on the (x, y) plane with a single fold that lifts trajectories in z and drops them back near the origin. As the control parameter c increases, the system undergoes a period-doubling cascade culminating in a strange attractor; past c ~ 5.7 the attractor develops a vertical lobe that gives it the "funnel" name.
- Hook: (placeholder hook, flagged)
- Review verdict: [first-pass] CLEAN (spot audit passed; no exposed bibkeys, files complete, invariants present)

### Chirikov Standard Map - KAM Tori
`playgrounds/bsc-y2s2/FIS2021-standard-map-kam` | FIS2021 | SHIPPED | tier:simple

- What it does / physics / visuals: The standard map is the discrete-time area-preserving map p_{n+1} = p_n + K sin(theta_n) (mod 2 pi) theta_{n+1} = theta_n + p_{n+1} (mod 2 pi) on the torus (theta, p) in [0, 2 pi)^2. It is the Poincare section of a periodically kicked rotator. At K = 0 the dynamics is integrable; at finite K the KAM theorem guarantees that sufficiently irrational tori survive while rational ones break into chains of resonance islands. The golden-mean torus breaks last, at K_crit ~ 0.9716 (Greene 1979).
- Hook: (placeholder hook, flagged)
- Review verdict: [first-pass] CLEAN (spot audit passed; no exposed bibkeys, files complete, invariants present)

### Van der Pol: Limit Cycle to Relaxation Oscillator
`playgrounds/bsc-y2s2/FIS2021-van-der-pol-relaxation` | FIS2021 | SHIPPED | tier:simple

- What it does / physics / visuals: The Van der Pol equation: x'' - mu (1 - x^2) x' + x = 0
- Hook: (placeholder hook, flagged)
- Review verdict: [deep] RENDER-NEUTRAL TEXT FIX ONLY
- Flags to address:
...
`playgrounds/bsc-y3s1/AST3014-alfven-wave-mhd-1d` | AST3014 | SHIPPED | tier:simple

- What it does / physics / visuals: Transverse magnetic perturbation travels at $v_A = B_0/\sqrt{\mu_0 \rho}$; magnetic-tension restoring force. Source: Goedbloed-Poedts Ch. 5 (`goedbloed-plasma`).
- Hook: (placeholder hook, flagged)
- Review verdict: [deep] RENDER-NEUTRAL TEXT FIX ONLY
- Flags to address:
...
`playgrounds/bsc-y3s1/AST3014-bondi-accretion-spherical` | AST3014 | SHIPPED | tier:simple

- What it does / physics / visuals: Bondi radius and accretion rate for a steady spherically symmetric inflow; sonic point at $r_B/2$. Source: Frank-King-Raine Ch. 2 (`frank-king-raine`).
- Hook: (placeholder hook, flagged)
- Review verdict: [deep] NEEDS CODE FIX + RECAPTURE

### Fluid Painter: Lattice Boltzmann Sandbox
`playgrounds/bsc-y3s1/AST3014-fluid-painter-lattice-boltzmann` | AST3014 | SHIPPED | tier:large
...
### Parker Solar Wind
`playgrounds/bsc-y3s1/AST3014-parker-solar-wind` | AST3014 | SHIPPED | tier:simple
...
`playgrounds/bsc-y3s1/AST3014-plasma-waves-dispersion` | AST3014 | SHIPPED | tier:medium
...
`playgrounds/bsc-y3s1/AST3014-sedov-taylor-blastwave` | AST3014 | SHIPPED | tier:simple

- What it does / physics / visuals: Self-similar point-explosion blast: $R \propto (E t^2/\rho)^{1/5}$. Source: Shu Vol II Ch. 17 (`shu-vol2`).
- Hook: (placeholder hook, flagged)
- Review verdict: [deep] NEEDS CODE FIX + RECAPTURE

### Single-Particle Motion: Drifts in E and B
`playgrounds/bsc-y3s1/AST3014-single-particle-em-drift-3d` | AST3014 | SHIPPED | tier:medium

- What it does / physics / visuals: An interactive single-particle picture of charged-particle motion in static electric and magnetic fields, m dv/dt = q(E + v x B), integrated with the Boris pusher (a time-reversible leapfrog that conserves speed exactly in a pure magnetic field). Presets show the cyclotron helix (omega_c = qB/m), the E x B drift (v_d = E x B / B^2, the same for every charge and mass), the grad-B and curvature drifts, and a magnetic mirror that reflects the particle while the adiabatic invariant mu = m v_perp^2 / 2B is conserved. The 3D orbit is drawn with a Canvas2D orthographic projection (deterministic and gate-robust; honestly a projection of the real 3D trajectory, not a WebGL scene). The headline readouts are |v| (constant to 1e-9 in pure B, the Boris property) and mu. Physics is the gate-tested shared sim.js.
...
`playgrounds/bsc-y3s1/AST3014-sph-sod-shock-tube` | AST3014 | SHIPPED | tier:simple

- What it does / physics / visuals: The Sod shock tube is the canonical compressible-fluid benchmark. A membrane at x = 0.5 separates two states of an ideal gas with gamma = 1.4:
- Hook: (placeholder hook, flagged)
- Review verdict: [first-pass] RENDER-NEUTRAL TEXT FIX ONLY
- Flags to address:
...
`playgrounds/bsc-y3s1/AST3014-two-stream-pic-plasma` | AST3014 | SHIPPED | tier:medium
...
### Aperture Photometry
`playgrounds/bsc-y3s1/AST3015-aperture-photometry-toy` | AST3015 | SHIPPED | tier:simple

- What it does / physics / visuals: Synthetic Moffat PSF on a CCD; aperture + sky annulus recovers true flux. Source: Howell CCD Handbook (`howell-ccd`).
- Hook: (placeholder hook, flagged)
- Review verdict: [deep] RENDER-NEUTRAL TEXT FIX ONLY

### Kepler Equation Newton Iteration
`playgrounds/bsc-y3s1/AST3015-kepler-equation-newton-iteration` | AST3015 | SHIPPED | tier:simple

- What it does / physics / visuals: The Kepler equation $M = E - e \sin E$ relates the mean anomaly $M$ (the linear angular coordinate that ticks at $2\pi$ per orbital period) to the eccentric anomaly $E$ (the angle of the planet on the auxiliary circle of the ellipse). Position on the orbit follows from $(a(\cos E - e), \, a\sqrt{1-e^2} \sin E)$. The equation is transcendental, so we solve it numerically.
- Hook: (placeholder hook, flagged)
- Review verdict: [first-pass] RENDER-NEUTRAL TEXT FIX ONLY
- Flags to address:
  - [blocker] spec.md contains placeholder markers: hook = 'STATUS: needs_hook', one_paragraph = 'STATUS: needs_paragraph'. Replace with actual descriptions.
...
`playgrounds/bsc-y3s1/AST3015-least-squares-orbit-fit-gauss` | AST3015 | SHIPPED | tier:simple

- What it does / physics / visuals: Noisy positions along a Kepler orbit; fit a circle by least squares. The fit is biased when $e > 0$. Source: Bate-Mueller-White Ch. 5 (`bmw`).
- Hook: (placeholder hook, flagged)
- Review verdict: [deep] RENDER-NEUTRAL TEXT FIX ONLY

### Particle-Mesh Self-Gravitating 2D Disk
`playgrounds/bsc-y3s1/AST3015-particle-mesh-2d-disk` | AST3015 | SHIPPED | tier:simple

- What it does / physics / visuals: A flat 2D disc of 1500 self-gravitating particles in an exponential surface-density profile. Gravity solved via particle-mesh on a 32 x 32 periodic grid using cloud-in-cell (CIC) deposit and interpolation.
- Hook: (placeholder hook, flagged)
- Review verdict: [first-pass] RENDER-NEUTRAL TEXT FIX ONLY
- Flags to address:
...
`playgrounds/bsc-y3s1/FIS3003-angular-momentum-coupling-3d` | FIS3003 | DRAFT | HERO-CANDIDATE | tier:advanced
...
`playgrounds/bsc-y3s1/FIS3003-heisenberg-uncertainty-visualizer` | FIS3003 | DRAFT | HERO-CANDIDATE | tier:hero

- What it does / physics / visuals: A quantum state shown in both conjugate spaces at once: the position wavefunction |psi(x)|^2 and its Fourier transform |phi(k)|^2, with the standard deviations sigma_x and sigma_p drawn as extent bars. A slow breathing modulates the squeeze so the seesaw is live, as the position packet narrows the momentum packet must broaden. A gauge tracks the product sigma_x sigma_p against the hbar/2 floor it can never cross; a Gaussian sits exactly on it, while a box, triangle or double-bump state sits strictly above. The momentum representation is the discrete Fourier transform of psi. The headless sim.js is gate-tested for the Gaussian saturation, the universal bound, the squeeze tradeoff, the FT unitarity, shift/boost invariance and the scaling law.
...
`playgrounds/bsc-y3s1/FIS3003-hydrogen-atom-stark-zeeman` | FIS3003 | DRAFT | HERO-CANDIDATE | tier:advanced
...
`playgrounds/bsc-y3s1/FIS3003-spin-bloch-sphere-dynamics` | FIS3003 | SHIPPED | tier:advanced
...
`playgrounds/bsc-y3s1/FIS3008-renormalization-group-flow-1d` | FIS3008 | SHIPPED | HERO-CANDIDATE | tier:advanced
...
### Fabry-Perot Finesse
`playgrounds/bsc-y3s1/FIS3019-fabry-perot-finesse` | FIS3019 | SHIPPED | tier:simple
...
`playgrounds/bsc-y3s1/FIS3019-fabry-perot-spectrometer` | FIS3019 | SHIPPED | HERO-CANDIDATE | tier:advanced
...
`playgrounds/bsc-y3s1/FIS3019-fourier-optics-4f-system` | FIS3019 | SHIPPED | HERO-CANDIDATE | tier:hero
...
`playgrounds/bsc-y3s1/FIS3019-gaussian-beam-paraxial` | FIS3019 | SHIPPED | tier:simple

- What it does / physics / visuals: The fundamental TEM_00 mode of a laser cavity, modeled in the paraxial (slowly varying envelope) limit. The beam is narrowest at z = 0 with 1/e^2 intensity radius w_0; it expands hyperbolically along z as it propagates.
- Hook: (placeholder hook, flagged)
- Review verdict: [first-pass] RENDER-NEUTRAL TEXT FIX ONLY
- Flags to address:
  - [blocker] spec.md contains placeholder markers: hook and one_paragraph = 'STATUS: needs_hook/needs_paragraph'. Replace with actual text.
...
`playgrounds/bsc-y3s1/FIS3019-grating-resolving-power` | FIS3019 | SHIPPED | tier:simple

- What it does / physics / visuals: $N$-slit grating; principal maxima at $d \sin\theta = m\lambda$, resolving power $R = mN$. Source: Hecht Ch. 10 (`hecht2017`).
- Hook: (placeholder hook, flagged)
- Review verdict: [first-pass] NEEDS CODE FIX + RECAPTURE
- Flags to address:
  - [blocker] spec.md contains placeholder markers: hook and one_paragraph = 'STATUS: needs_hook/needs_paragraph'. Replace with actual text.
  - [medium] README.md is only 2 lines; expand to ~15 covering grating physics and resolving power concept.
  - [medium] index.html contains two raw bib keys. Move to spec or figcaption.
  - [high] Golden frames have only 2 unique MD5s out of 5 total; frames are duplicated. Animation progression not captured. Recapture needed.

### Gaussian Beam - ABCD Propagation
`playgrounds/bsc-y3s1/FIS3019-laser-gaussian-beam-propagation` | FIS3019 | SHIPPED | HERO-CANDIDATE | tier:advanced
...
`playgrounds/bsc-y3s1/FIS3019-michelson-interferometer` | FIS3019 | SHIPPED | tier:simple

- What it does / physics / visuals: A Michelson interferometer with one moving mirror produces a path difference $L = 2d$ between the two arms. The detector intensity is
- Hook: (placeholder hook, flagged)
- Review verdict: [first-pass] RENDER-NEUTRAL TEXT FIX ONLY
- Flags to address:
  - [blocker] spec.md contains placeholder markers: hook and one_paragraph = 'STATUS: needs_hook/needs_paragraph'. Replace with actual text.
...
`playgrounds/bsc-y3s1/FIS3019-polarization-jones-calculus` | FIS3019 | SHIPPED | HERO-CANDIDATE | tier:advanced
...
`playgrounds/bsc-y3s1/FIS3019-thin-film-interference` | FIS3019 | SHIPPED | tier:simple

- What it does / physics / visuals: A thin layer of refractive index n_film and thickness d sits on a substrate. Default values: n_top = 1.0 (air), n_film = 1.33 (water/oil), n_sub = 1.5 (glass). White light at normal incidence reflects from both interfaces; the two reflected beams interfere, with the interference pattern depending on d / lambda.
- Hook: (placeholder hook, flagged)
- Review verdict: [first-pass] RENDER-NEUTRAL TEXT FIX ONLY
- Flags to address:
  - [blocker] spec.md contains placeholder markers: hook and one_paragraph = 'STATUS: needs_hook/needs_paragraph'. Replace with actual text.
...
`playgrounds/bsc-y3s1/M3012-fourier-epicycle-drawing` | M3012 | SHIPPED | tier:medium
...
`playgrounds/bsc-y3s1/M3012-fourier-vs-laplace-transform-pair` | M3012 | SHIPPED | tier:simple

- What it does / physics / visuals: Side-by-side time-domain, $|F(\omega)|^2$ and $F(s)$ with pole map. Source: Arfken-Weber Ch. 15 (`arfken-weber`).
- Hook: (placeholder hook, flagged)
- Review verdict: [first-pass] NEEDS CODE FIX + RECAPTURE
- Flags to address:
  - [blocker] spec.md contains placeholder markers: hook and one_paragraph = 'STATUS: needs_hook/needs_paragraph'. Replace with actual text.
  - [medium] README.md is only 2 lines; expand to ~15 covering Fourier-Laplace relationship.
  - [medium] index.html contains two raw bib keys. Move to spec or figcaption.
  - [high] Golden frames have only 2 unique MD5s out of 5 total; animation progression not captured. Recapture needed.

### 1D Green's Function for the Laplacian
`playgrounds/bsc-y3s1/M3012-green-function-1d-laplacian` | M3012 | SHIPPED | tier:simple

- What it does / physics / visuals: Tent-shaped $G(x, x_0)$ with Dirichlet BC; convolution gives $u(x) = \int G f dx'$. Source: Arfken-Weber Ch. 9 (`arfken-weber`).
- Hook: (placeholder hook, flagged)
- Review verdict: [first-pass] NEEDS CODE FIX + RECAPTURE
- Flags to address:
  - [blocker] spec.md contains placeholder markers: hook and one_paragraph = 'STATUS: needs_hook/needs_paragraph'. Replace with actual text.
  - [medium] README.md is only 2 lines; expand to ~15 covering Green's function concept.
  - [medium] index.html contains two raw bib keys. Move to spec or figcaption.
  - [high] Golden frames have only 1 unique MD5 out of 5 total; all five frames appear identical. Animation progression completely absent. Recapture needed.

### Sturm-Liouville Eigenfunctions
`playgrounds/bsc-y3s1/M3012-sturm-liouville-eigenfunctions` | M3012 | SHIPPED | tier:simple
...
`playgrounds/bsc-y3s2/AST3016-bremsstrahlung-spectrum` | AST3016 | SHIPPED | tier:simple

- What it does / physics / visuals: Flat below $h\nu = kT$, exponential cutoff above. Source: Rybicki-Lightman Ch. 5 (`rybickilightman1979`).
- Hook: (placeholder hook, flagged)
- Review verdict: [first-pass] RENDER-NEUTRAL TEXT FIX ONLY
- Flags to address:
  - [blocker] spec.md contains placeholder markers: hook and one_paragraph = 'STATUS: needs_hook/needs_paragraph'. Replace with actual text.
...
`playgrounds/bsc-y3s2/AST3016-radiative-transfer-1d-slab` | AST3016 | SHIPPED | tier:simple
...
- Flags to address:
  - [blocker] spec.md contains placeholder markers: hook and one_paragraph = 'STATUS: needs_hook/needs_paragraph'. Replace with actual text.
  - [medium] README.md is only 2 lines; expand to ~15 covering radiative transfer concept.
  - [medium] index.html contains two raw bib keys. Move to spec or figcaption.
  - [high] Golden frames have only 1 unique MD5 out of 5 total; all frames appear identical. Animation progression absent. Recapture needed.

### Relativistic Beaming Pattern
`playgrounds/bsc-y3s2/AST3016-relativistic-beaming-azimuth` | AST3016 | SHIPPED | tier:simple
...
- Flags to address:
  - Card text: spec.md has placeholder strings: hook = `'STATUS: needs_hook'` and one_paragraph = `'STATUS: needs_paragraph'`. These render on the gallery card and must be replaced with actual prose.
  - User-facing text: README.md and index.html figcaption expose raw BibTeX cite keys in backticks: `(rybickilightman1979)`. Should be formatted as "Rybicki & Lightman 1979, Ch. 6" or with a hyperlink.
  - Golden frames legible: Yes. Yellow spectrum curve and blue reference-slope line are sharp, electron animation (right panel) is clear, N(gamma) distribution (lower right) is readable. No garble or off-
  - Axes and units: Axes are labeled "log ν (Hz)" and "log F" correctly. Electron parameters (gamma, B, p) are shown at the top of each frame with correct units (G for Gauss, dimensionless for p).
...
- Review verdict: [first-pass] NEEDS CODE FIX + RECAPTURE
- Flags to address:
  - [CRITICAL] All 5 golden frames are byte-identical (104606 bytes each). Visual inspection confirms all 5 frames show the same state: eta=6.10, circles all at the same position on the Planck line. The p
  - [HIGH] Placeholder hook and one_paragraph in spec.md: `STATUS: needs_hook` and `STATUS: needs_paragraph` are not replaced.
  - [HIGH] Raw BibTeX keys in user-facing HTML and README: figcaption and README contain `(liddle-cosmology)` in backticks. Expose the proper citation format, not the raw key.
  - [MEDIUM] No visible slider or control interaction documented in the capture. The playground has a slider (eta) but the golden frames are static. Need either (a) animated capture with CAPTURE_FRAC driv
...
- Review verdict: [first-pass] NEEDS CODE FIX + RECAPTURE
- Flags to address:
  - [CRITICAL] All 5 golden frames are byte-identical (89428 bytes each). Playground has three sigma sliders (s1, s2, s3) but no capture-time variation. Golden frames do not demonstrate the interactive ru
  - [HIGH] Placeholder hook and one_paragraph in spec.md not removed.
  - [HIGH] Raw BibTeX keys in spec and HTML: `(carroll-ostlie)` in spec title line and index.html figcaption.
  - [MEDIUM] Missing readout element: spec tags list "live-readout" but HTML has no visible readout div (unlike synchrotron-spectrum and bbn-light-element-toy which have readout elements).

### Friedmann Cosmography
`playgrounds/bsc-y3s2/AST3017-friedmann-cosmography` | AST3017 | SHIPPED | tier:simple
...
`playgrounds/bsc-y3s2/AST3017-gravitational-lensing-caustics` | AST3017 | SHIPPED | tier:large
...
`playgrounds/bsc-y3s2/AST3017-gravitational-microlensing-event` | AST3017 | SHIPPED | tier:large
...
- Flags to address:
  - [CRITICAL BLOCKER] Spec lists 'live-readout' tag but HTML has zero readout elements. Gate requires visible live invariant readout; this playground has none. Either remove 'live-readout' tag from spec 
  - [HIGH] Placeholder hook and one_paragraph in spec.md must be replaced with actual prose.
  - [MEDIUM] Frames are distinct (5 different sizes), indicating animation/parameter sweep works; no capture defect.

### Gravitational-Wave Chirp Sonification
`playgrounds/bsc-y3s2/AST3017-gravitational-wave-chirp-sonification` | AST3017 | SHIPPED | tier:large
...
- Review verdict: [first-pass] NEEDS CODE FIX + RECAPTURE
- Flags to address:
  - [HIGH] Frame sizes not all distinct (distinct_sizes=3). Multiple frames are byte-identical, indicating incomplete photon trajectory animation. Capture should sweep impact parameter or observer positio
  - [HIGH] Placeholder hook and one_paragraph in spec.md not removed.
  - [MEDIUM] 7 readout elements detected but spec does not list 'live-readout' tag (though hook says "watch", implying animation). Update spec tags to include 'live-readout' if readouts are active, or pru

### Shapiro Time Delay
`playgrounds/bsc-y3s2/AST3017-shapiro-time-delay` | AST3017 | SHIPPED | tier:simple

- What it does / physics / visuals: A light signal travels past a massive body (Schwarzschild M = 1 in geometric units) at impact parameter b. The leading-order PPN time delay relative to flat-space is
- Hook: (placeholder hook, flagged)
- Review verdict: [first-pass] RENDER-NEUTRAL TEXT FIX ONLY
- Flags to address:
  - [HIGH] Placeholder hook and one_paragraph in spec.md not removed.
...
`playgrounds/bsc-y3s2/FIS3005-band-structure-tight-binding` | FIS3005 | SHIPPED | HERO-CANDIDATE | tier:advanced
...
### The p-n Junction
`playgrounds/bsc-y3s2/FIS3005-semiconductor-pn-junction` | FIS3005 | SHIPPED | HERO-CANDIDATE | tier:hero
...
### The Meissner Effect
`playgrounds/bsc-y3s2/FIS3005-superconductivity-meissner-3d` | FIS3005 | SHIPPED | HERO-CANDIDATE | tier:advanced
...
`playgrounds/bsc-y3s2/FIS3020-bcs-gap-self-consistent` | FIS3020 | SHIPPED | tier:simple

- What it does / physics / visuals: $\Delta(T)$ from the BCS gap equation; universal ratio $2\Delta_0 / k_B T_c \approx 3.53$. Source: Ashcroft-Mermin Ch. 34 (`ashcroft-mermin`).
- Hook: (placeholder hook, flagged)
- Review verdict: [first-pass] NEEDS CODE FIX + RECAPTURE
- Flags to address:
  - [CRITICAL] All 5 golden frames byte-identical. Playground animates BCS superconducting gap equation but capture is static. CAPTURE_FRAC must drive a control parameter (temperature T, coupling strength
  - [HIGH] Placeholder hook and one_paragraph in spec.md not removed.
  - [HIGH] Raw BibTeX keys in HTML: 2 citations in backticks.

### Bloch Oscillations
`playgrounds/bsc-y3s2/FIS3020-bloch-oscillations` | FIS3020 | SHIPPED | tier:simple

- What it does / physics / visuals: Particle in a tilted cosine band; quasi-momentum slides through the BZ and Bragg-reflects. Source: Ashcroft-Mermin Ch. 12 (`ashcroft-mermin`).
- Hook: (placeholder hook, flagged)
- Review verdict: [first-pass] NEEDS CODE FIX + RECAPTURE
- Flags to address:
  - [CRITICAL] All 5 golden frames byte-identical. Bloch oscillations are time-dependent dynamics (electron position oscillates as it moves through band structure in an external field) but capture is stat
  - [HIGH] Placeholder hook and one_paragraph in spec.md.
  - [HIGH] Raw BibTeX keys: 2 in HTML.

### Cooper Pair Binding Energy
`playgrounds/bsc-y3s2/FIS3020-cooper-pair-binding-energy` | FIS3020 | SHIPPED | tier:simple
...
`playgrounds/bsc-y3s2/FIS3020-fermi-surface-2d-square` | FIS3020 | SHIPPED | tier:simple

- What it does / physics / visuals: Tight-binding electrons on a square lattice with nearest-neighbor hopping. Dispersion $E(k_x, k_y) = -2t (\cos k_x + \cos k_y)$ over the Brillouin zone $(k_x, k_y) \in [-\pi, \pi]^2$. The bandwidth is $8t$ from $-4t$ (Gamma point) to $+4t$ (M point).
- Hook: (placeholder hook, flagged)
- Review verdict: [first-pass] RENDER-NEUTRAL TEXT FIX ONLY
- Flags to address:
...
`playgrounds/bsc-y3s2/FIS3020-phonon-dispersion-1d-monatomic-diatomic` | FIS3020 | SHIPPED | tier:simple

- What it does / physics / visuals: Monatomic acoustic vs diatomic acoustic + optical; gap at zone boundary. Source: Ashcroft-Mermin Ch. 22 (`ashcroft-mermin`).
- Hook: (placeholder hook, flagged)
- Review verdict: [first-pass] RENDER-NEUTRAL TEXT FIX ONLY
- Flags to address:
...
`playgrounds/bsc-y3s2/FIS3028-aberration-of-light-stellar` | FIS3028 | SHIPPED | tier:simple

- What it does / physics / visuals: Stars at uniform angular positions in their rest frame; the observer moves at velocity $\beta c$ along $+x$. Lorentz aberration
- Hook: (placeholder hook, flagged)
- Review verdict: [first-pass] RENDER-NEUTRAL TEXT FIX ONLY
- Flags to address:
...
`playgrounds/bsc-y3s2/FIS3028-relativistic-collision-mandelstam` | FIS3028 | SHIPPED | tier:simple

- What it does / physics / visuals: $\sqrt s \propto \sqrt{E_{lab}}$ for fixed targets, but $\propto E_{lab}$ for symmetric colliders. Source: Griffiths Particles Ch. 3 (`griffiths-particles`).
- Hook: (placeholder hook, flagged)
- Review verdict: [first-pass] NEEDS CODE FIX + RECAPTURE
- Flags to address:
  - [CRITICAL] All 5 golden frames byte-identical. Mandelstam variables (s, t, u) describe 2-body collisions and depend on energy/momentum. CAPTURE_FRAC should drive collision energy or angle so Mandelsta
  - [HIGH] Placeholder hook and one_paragraph in spec.md.
  - [HIGH] Raw BibTeX keys: 2 in HTML.

### Relativistic Doppler Effect
`playgrounds/bsc-y3s2/FIS3028-relativistic-doppler` | FIS3028 | SHIPPED | tier:simple

- What it does / physics / visuals: A monochromatic source at rest in frame $K'$ emits frequency $f_s$ and moves at velocity $\beta c$ along the $+x$ axis relative to the observer in frame $K$. The observer sees the photon arriving at angle $\theta$ from the $+x$ axis. The relativistic Doppler factor is
- Hook: (placeholder hook, flagged)
- Review verdict: [first-pass] RENDER-NEUTRAL TEXT FIX ONLY
- Flags to address:
  - [HIGH] Placeholder hook in spec.md: `STATUS: needs_hook` not removed (one_paragraph is filled).
...
### Thomas Precession
`playgrounds/bsc-y3s2/FIS3028-thomas-precession` | FIS3028 | SHIPPED | tier:simple

- What it does / physics / visuals: A gyroscope on a circular orbit picks up $(\gamma - 1)$ rad of extra rotation per revolution. Source: Jackson 3e Ch. 11.8 (`jackson3e`).
- Hook: (placeholder hook, flagged)
- Review verdict: [first-pass] NEEDS CODE FIX + RECAPTURE
- Flags to address:
  - [CRITICAL] All 5 golden frames byte-identical. Thomas precession is a relativistic dynamics effect; precession rate depends on velocity. CAPTURE_FRAC should vary velocity so precession angle and spin 
  - [HIGH] Placeholder hook and one_paragraph in spec.md.
  - [HIGH] Raw BibTeX keys: 2 in HTML.

### 1D TDSE Wavepacket Scattering
`playgrounds/bsc-y3s2/FIS3029-1d-tdse-scattering-comparator` | FIS3029 | SHIPPED | tier:simple

- What it does / physics / visuals: A 1D Gaussian wavepacket initially at x_0 = -15 with momentum k_0 moves to the right and scatters off a potential (rectangular barrier, step, or square well). Computed with Crank-Nicolson on a uniform grid; norm-preserving by construction.
- Hook: (placeholder hook, flagged)
...
`playgrounds/bsc-y3s2/FIS3029-addition-of-angular-momenta` | FIS3029 | SHIPPED | tier:simple

- What it does / physics / visuals: $j_1 \otimes j_2 = |j_1-j_2| \oplus \dots \oplus j_1+j_2$. Source: Sakurai QM Ch. 3 (`sakurai-qm`).
- Hook: (placeholder hook, flagged)
- Review verdict: [first-pass] NEEDS CODE FIX + RECAPTURE
- Flags to address:
  - [CRITICAL] All 5 golden frames byte-identical. Clebsch-Gordan coefficients depend on j1, j2, and their coupling. CAPTURE_FRAC should vary angular momenta (e.g., j1 from 0 to 2) so coupling diagram and
  - [HIGH] Placeholder hook and one_paragraph in spec.md.
  - [HIGH] Raw BibTeX keys: 2 in HTML.

### Aharonov-Bohm Effect
`playgrounds/bsc-y3s2/FIS3029-aharonov-bohm-flux-line` | FIS3029 | SHIPPED | tier:simple

- What it does / physics / visuals: Solenoid behind a double slit shifts the fringe pattern by $\Phi/\Phi_0$ cycles. Source: Sakurai Ch. 2 (`sakurai-qm`).
- Hook: (placeholder hook, flagged)
- Review verdict: [first-pass] NEEDS CODE FIX + RECAPTURE
- Flags to address:
  - [CRITICAL] All 5 golden frames byte-identical. Aharonov-Bohm phase depends on enclosed magnetic flux. CAPTURE_FRAC should vary flux so phase difference and interference pattern change visibly.
  - [HIGH] Placeholder hook and one_paragraph in spec.md.
  - [HIGH] Raw BibTeX keys: 2 in HTML.

### Hydrogen Fine Structure
`playgrounds/bsc-y3s2/FIS3029-fine-structure-hydrogen` | FIS3029 | SHIPPED | tier:simple

- What it does / physics / visuals: Bohr levels split by $\alpha^2$ corrections; degeneracy is $j$-labeled. Source: Griffiths QM Ch. 6 (`griffiths-qm`).
- Hook: (placeholder hook, flagged)
- Review verdict: [first-pass] NEEDS CODE FIX + RECAPTURE
- Flags to address:
  - [CRITICAL] All 5 golden frames byte-identical. Fine structure splits depend on n, l, j quantum numbers (spin-orbit coupling strength). CAPTURE_FRAC should vary quantum numbers or coupling strength so 
  - [HIGH] Placeholder hook and one_paragraph in spec.md.
  - [HIGH] Raw BibTeX keys: 2 in HTML.

### Hydrogen Orbital Cross Sections in the (x, z) Plane
`playgrounds/bsc-y3s2/FIS3029-hydrogen-orbital-cross-sections-2d` | FIS3029 | SHIPPED | tier:simple

- What it does / physics / visuals: The bound stationary states of the hydrogen atom, parameterized by three quantum numbers (n, l, m). Probability density |psi_nlm|^2 plotted in the plane through the nucleus that contains the z axis (i.e., y = 0). This is the standard textbook visualization for orbital shapes.
- Hook: (placeholder hook, flagged)
- Review verdict: [first-pass] RENDER-NEUTRAL TEXT FIX ONLY + MISSING READOUT
- Flags to address:
  - [CRITICAL BLOCKER] Spec lists 'live-readout' tag but HTML has zero readout elements. Gate requires visible invariant readout.
...
`playgrounds/bsc-y3s2/FIS3029-kronig-penney-bands` | FIS3029 | SHIPPED | tier:simple

- What it does / physics / visuals: A 1D crystal with delta-function spikes on a periodic lattice (period a, dimensionless strength P). The energy spectrum splits into allowed bands and forbidden gaps. The simplest textbook model in solid-state physics that produces a band structure.
- Hook: (placeholder hook, flagged)
- Review verdict: [first-pass] RENDER-NEUTRAL TEXT FIX ONLY
- Flags to address:
  - [HIGH] Placeholder hook in spec.md: `STATUS: needs_hook` not removed (one_paragraph is filled).
...
`playgrounds/bsc-y3s2/FIS3029-particle-in-a-well-zoo` | FIS3029 | SHIPPED | tier:simple

- What it does / physics / visuals: Three canonical 1D quantum bound-state problems plotted on the same axes for comparison: infinite square well, finite square well, and harmonic oscillator. In each, V(x) is fixed and we solve the time-independent Schrodinger equation for energy eigenstates.
- Hook: (placeholder hook, flagged)
- Review verdict: [deep] RENDER-NEUTRAL TEXT FIX ONLY
- Flags to address:
...
`playgrounds/bsc-y3s2/FIS3029-quantum-confinement-nanostructure` | FIS3029 | SHIPPED | tier:medium
...
`playgrounds/bsc-y3s2/FIS3029-quantum-random-walk` | FIS3029 | SHIPPED | tier:medium
...
`playgrounds/bsc-y3s2/FIS3029-wkb-vs-shooting` | FIS3029 | SHIPPED | tier:simple

- What it does / physics / visuals: Bound-state energies for a 1D particle in a power-law well V(x) = |x|^p / p, hbar = m = 1. Compare the Bohr-Sommerfeld (WKB) approximation to the "exact" reference levels for the harmonic oscillator (p = 2; closed form E_n = n + 1/2) and quartic anharmonic oscillator (p = 4; Bender-Wu 1969 numerical levels).
- Hook: (placeholder hook, flagged)
- Review verdict: [deep] RENDER-NEUTRAL TEXT FIX ONLY
- Flags to address:
...
`playgrounds/bsc-y3s2/FIS3029-zeeman-paschen-back-crossover` | FIS3029 | SHIPPED | tier:simple

- What it does / physics / visuals: Low-$B$ Zeeman: $g_J m_J \mu_B B$. High-$B$ Paschen-Back: $(m_L + 2m_S) \mu_B B$. Source: Griffiths QM Ch. 6.4 (`griffiths-qm`).
- Hook: (placeholder hook, flagged)
- Review verdict: [deep] NEEDS CODE FIX + RECAPTURE
- Flags to address:
  - spec.md line 12: `hook: 'STATUS: needs_hook'` (placeholder).
  - spec.md line 13: `one_paragraph: 'STATUS: needs_paragraph'` (placeholder).
  - Physical setup section (LS-coupling, Zeeman/Paschen-Back regimes).
  - Governing equations (TISE + H_mag term).

### Alpha Decay via Gamow Tunneling
`playgrounds/bsc-y3s2/FIS3030-alpha-decay-gamow-tunneling` | FIS3030 | SHIPPED | tier:simple
...
`playgrounds/bsc-y3s2/FIS3030-ckm-mixing-unitarity-triangle` | FIS3030 | SHIPPED | tier:simple
...
`playgrounds/bsc-y3s2/FIS3030-cosmic-ray-air-shower` | FIS3030 | SHIPPED | tier:large
...
`playgrounds/bsc-y3s2/FIS3030-nuclear-beta-decay-fermi-vs-gt` | FIS3030 | SHIPPED | tier:simple

- What it does / physics / visuals: Selection rules and Kurie plot. Source: Krane Nuclear Physics Ch. 9 (`krane-nuclear`).
- Hook: (placeholder hook, flagged)
- Review verdict: [deep] RENDER-NEUTRAL TEXT FIX ONLY
- Flags to address:
...
`playgrounds/bsc-y3s2/FIS3030-parton-distribution-toy` | FIS3030 | SHIPPED | tier:simple

- What it does / physics / visuals: $x f(x)$ for $u_v$, $d_v$, gluon, sea quarks. Source: Griffiths-Particles Ch. 9 (`griffiths-particles`).
- Hook: (placeholder hook, flagged)
- Review verdict: [deep] RENDER-NEUTRAL TEXT FIX ONLY
- Flags to address:
  - spec.md placeholder hook/one_paragraph.
...
`playgrounds/bsc-y3s2/FIS3030-semi-empirical-mass-formula` | FIS3030 | SHIPPED | tier:simple

- What it does / physics / visuals: The nuclear binding energy is the sum of five terms:
- Hook: (placeholder hook, flagged)
- Review verdict: [deep] RENDER-NEUTRAL TEXT FIX ONLY
- Flags to address:
  - spec.md placeholder hook/one_paragraph.
...
`playgrounds/bsc-y3s2/FIS4026-gmr-spin-valve-simulator` | FIS4026 | SHIPPED | HERO-CANDIDATE | tier:hero
...
`playgrounds/bsc-y3s2/FIS4026-mosfet-operation-animated` | FIS4026 | SHIPPED | HERO-CANDIDATE | tier:hero
...
`playgrounds/bsc-y3s2/FIS4026-solar-cell-generation-iv` | FIS4026 | SHIPPED | HERO-CANDIDATE | tier:hero
...
`playgrounds/bsc-y3s2/FIS4035-jaynes-cummings-model` | FIS4035 | SHIPPED | HERO-CANDIDATE | tier:hero
...
`playgrounds/bsc-y3s2/FIS4035-laser-rate-equations-dynamics` | FIS4035 | SHIPPED | HERO-CANDIDATE | tier:hero
...
`playgrounds/bsc-y3s2/FIS4035-nonlinear-optics-shg` | FIS4035 | SHIPPED | HERO-CANDIDATE | tier:hero
...
`playgrounds/bsc-y3s2/FIS4035-optical-fiber-modes-dispersion` | FIS4035 | SHIPPED | HERO-CANDIDATE | tier:hero
...
`playgrounds/bsc-y3s2/M3007-curvature-tensor-2d-surfaces` | M3007 | SHIPPED | tier:simple

- What it does / physics / visuals: Sphere, cylinder, hyperbolic plane, and torus. Source: Riley-Hobson Ch. 26 (`riley-hobson`).
- Hook: (placeholder hook, flagged)
- Review verdict: [deep] RENDER-NEUTRAL TEXT FIX ONLY
- Flags to address:
  - spec.md placeholder hook/one_paragraph.
...
`playgrounds/bsc-y3s2/M3007-geodesic-deviation-equation` | M3007 | SHIPPED | tier:simple

- What it does / physics / visuals: Two parallel-starting geodesics on a sphere converge at the pole. Source: Carroll Spacetime and Geometry Ch. 3 (`carroll-spacetime`).
- Hook: (placeholder hook, flagged)
- Review verdict: [deep] RENDER-NEUTRAL TEXT FIX ONLY
- Flags to address:
  - spec.md placeholder hook/one_paragraph.
...
`playgrounds/bsc-y3s2/M3007-parallel-transport-on-sphere` | M3007 | SHIPPED | tier:simple

- What it does / physics / visuals: A spherical triangle on the unit sphere with one vertex at the north pole and the other two at colatitude $\alpha$ separated by longitude $\beta$. Parallel-transporting a vector around the triangle rotates it by the enclosed solid angle $$\Omega = (1 - \cos\alpha) \beta.$$
- Hook: (placeholder hook, flagged)
- Review verdict: [deep] RENDER-NEUTRAL TEXT FIX ONLY
- Flags to address:
  - spec.md placeholder hook/one_paragraph.
...
`playgrounds/bsc-y3s2/MEF-afm-stm-surface-interaction` | MEF | SHIPPED | tier:medium
...
`playgrounds/bsc-y3s2/MEF-nanofabrication-lithography-resolution` | MEF | SHIPPED | tier:medium
...
`playgrounds/msc-y1/M2009-fourier-series-convergence-gibb` | M2009 | SHIPPED | tier:standard
...
`playgrounds/msc-y1/M2009-green-function-propagator` | M2009 | SHIPPED | tier:standard
...
`playgrounds/msc-y1/M2009-pde-zoo-interactive` | M2009 | SHIPPED | HERO-CANDIDATE | tier:hero
...
`playgrounds/msc-y1/MAA-AB-habitable-zone-stellar-flux` | MAA-AB | SHIPPED | tier:simple

- What it does / physics / visuals: A central star characterized by effective temperature $T_\text{eff}$ and radius $R_\star$ (in solar units). The luminosity is the Stefan-Boltzmann integral
- Hook: (placeholder hook, flagged)
- Review verdict: [deep] RENDER-NEUTRAL TEXT FIX ONLY
- Flags to address:
  - spec.md placeholder hook/one_paragraph.
...
`playgrounds/msc-y1/MAA-AS-asymptotic-period-spacing` | MAA-AS | SHIPPED | tier:simple

- What it does / physics / visuals: $\Pi_1$ distinguishes RGB (~80 s) from RC (~250 s). Source: Aerts et al. Ch. 3 (`aerts-asteroseism`).
- Hook: (placeholder hook, flagged)
- Review verdict: [deep] RENDER-NEUTRAL TEXT FIX ONLY
- Flags to address:
  - spec.md placeholder hook/one_paragraph.
...
`playgrounds/msc-y1/MAA-AS-mode-trapping-evolved-stars` | MAA-AS | SHIPPED | tier:simple

- What it does / physics / visuals: Periodic ΔP modulation from a buoyancy-frequency glitch. Source: Mosser et al. 2018 (`mosser2018-trap`); Aerts et al. Ch. 3 (`aerts-asteroseism`).
- Hook: (placeholder hook, flagged)
- Review verdict: [deep] RENDER-NEUTRAL TEXT FIX ONLY
- Flags to address:
  - spec.md placeholder hook/one_paragraph.
...
`playgrounds/msc-y1/MAA-AS-stellar-oscillation-modes` | MAA-AS | SHIPPED | tier:large
...
`playgrounds/msc-y1/MAA-CO-cosmic-distance-ladder` | MAA-CO | SHIPPED | tier:medium
...
`playgrounds/msc-y1/MAA-CO-slow-roll-inflation` | MAA-CO | SHIPPED | tier:medium
...
`playgrounds/msc-y1/MAA-CS-matter-radiation-equality` | MAA-CS | SHIPPED | tier:simple
...
`playgrounds/msc-y1/MAA-DM-em-on-gmm-2d` | MAA-DM | SHIPPED | tier:simple

- What it does / physics / visuals: A 2D scatter of N = 600 points drawn from a 3-component Gaussian mixture with known means, covariances, and mixing weights. The EM algorithm tries to recover those parameters using only the data, alternating soft cluster assignment (E-step) and parameter refit (M-step).
- Hook: (placeholder hook, flagged)
- Review verdict: [first-pass] RENDER-NEUTRAL TEXT FIX ONLY
- Flags to address:
...
### GP Kernel Zoo
`playgrounds/msc-y1/MAA-DM-gp-kernel-zoo` | MAA-DM | SHIPPED | tier:simple

- What it does / physics / visuals: A 1D Gaussian Process: a probability distribution over functions. Five kernels (RBF, Matern 3/2, Matern 5/2, periodic, linear) parameterized by length scale and amplitude. Top panel: prior samples (no data). Bottom panel: posterior conditioned on observations with noise sigma_n.
- Hook: (placeholder hook, flagged)
- Review verdict: [first-pass] RENDER-NEUTRAL TEXT FIX ONLY
- Flags to address:
...
`playgrounds/msc-y1/MAA-DM-mean-field-vi-on-banana` | MAA-DM | SHIPPED | tier:simple

- What it does / physics / visuals: Fit a mean-field Gaussian q(x, y) = N(mu_x, sigma_x^2) * N(mu_y, sigma_y^2) to a Rosenbrock-style banana target. The banana is a long curved valley; the mean-field Gaussian is axis-aligned; this gap is the canonical failure mode of variational inference.
- Hook: (placeholder hook, flagged)
- Review verdict: [first-pass] RENDER-NEUTRAL TEXT FIX ONLY
- Flags to address:
...
`playgrounds/msc-y1/MAA-GD-chandrasekhar-dynamical-friction` | MAA-GD | SHIPPED | tier:large
...
### Chandrasekhar Dynamical Friction
`playgrounds/msc-y1/MAA-GD-dynamical-friction-chandrasekhar` | MAA-GD | SHIPPED | tier:simple
...
`playgrounds/msc-y1/MAA-GD-galaxy-merger-nbody` | MAA-GD | SHIPPED | tier:large

- What it does / physics / visuals: Each tracer feels analytic Hernquist potentials of BOTH halos while halo centers integrate as a softened 2-body. Plummer softening keeps the pair force finite during close encounters.
...
`playgrounds/msc-y1/MAA-GD-jeans-isothermal-sphere` | MAA-GD | SHIPPED | tier:simple
...
`playgrounds/msc-y1/MAA-GD-orbits-in-axisymmetric-potential` | MAA-GD | SHIPPED | tier:simple
...
`playgrounds/msc-y1/MAA-GD-spiral-density-wave-dispersion` | MAA-GD | SHIPPED | tier:simple
...
`playgrounds/msc-y1/MAA-HE-inverse-compton-scattering-cooling` | MAA-HE | SHIPPED | tier:simple

- What it does / physics / visuals: Relativistic electrons (Lorentz factor $\gamma$) immersed in a soft-photon bath (energy density $U_\text{ph}$) lose energy by inverse-Compton up-scattering of the photons. In the Thomson limit the cooling time is $$t_\text{IC} = \frac{3 m_e c}{4 \sigma_T \gamma U_\text{ph}}.$$
- Hook: (placeholder hook, flagged)
- Review verdict: [first-pass] RENDER-NEUTRAL TEXT FIX ONLY
- Flags to address:
...
`playgrounds/msc-y1/MAA-HE-pulsar-wind-nebula-magnetization` | MAA-HE | SHIPPED | tier:simple
...
`playgrounds/msc-y1/MAA-OT-aperture-synthesis-uv-plane` | MAA-OT | SHIPPED | tier:large
...
`playgrounds/msc-y1/MAA-OT-point-spread-function-strehl` | MAA-OT | SHIPPED | tier:simple
...
`playgrounds/msc-y1/MAA-OT-pulsar-dispersion-measure` | MAA-OT | SHIPPED | tier:medium
...
`playgrounds/msc-y1/MAA-OT-speckle-pattern-statistics` | MAA-OT | SHIPPED | tier:simple
...
`playgrounds/msc-y1/MAA-SA-convection-mixing-length` | MAA-SA | SHIPPED | tier:simple
...
`playgrounds/msc-y1/MAA-SA-main-sequence-mass-luminosity` | MAA-SA | SHIPPED | tier:simple
...
`playgrounds/msc-y1/MAA-SA-nuclear-burning-rate-temperature` | MAA-SA | SHIPPED | tier:simple
...
`playgrounds/msc-y1/MAA-SA-polytrope-lane-emden` | MAA-SA | SHIPPED | tier:simple
...
`playgrounds/msc-y1/MAA-SE-binary-star-mass-transfer` | MAA-SE | SHIPPED | tier:standard
...
`playgrounds/msc-y1/MAA-SE-neutron-star-tov-equation` | MAA-SE | SHIPPED | tier:standard
...
`playgrounds/msc-y1/MAA-SE-stellar-structure-full-model` | MAA-SE | SHIPPED | HERO-CANDIDATE | tier:hero
...
`playgrounds/msc-y1/MAA-SP-eddington-grey-atmosphere` | MAA-SP | SHIPPED | tier:simple

- What it does / physics / visuals: A grey (frequency-independent opacity) stellar atmosphere in radiative equilibrium. The temperature profile follows $$T(\tau) = T_\text{eff} \left[\tfrac{3}{4} (\tau + \tfrac{2}{3})\right]^{1/4}.$$
- Hook: (placeholder hook, flagged)
- Review verdict: (not yet reviewed)

### Voigt Profile Decomposition
`playgrounds/msc-y1/MAA-SP-voigt-profile-decomposition` | MAA-SP | SHIPPED | tier:simple
...
`playgrounds/msc-y1/MAA-SS-resonance-mean-motion-toy` | MAA-SS | SHIPPED | tier:simple
...
`playgrounds/msc-y1/MAA-SS-secular-perturbation-laplace-lagrange` | MAA-SS | SHIPPED | tier:simple
...
### Stellar Habitable Zone
`playgrounds/msc-y1/MAA-SS-stellar-habitable-zone` | MAA-SS | SHIPPED | tier:medium
...
`playgrounds/msc-y1/MAA-ST-channel-capacity-bsc` | MAA-ST | SHIPPED | tier:simple

- What it does / physics / visuals: A BSC flips each transmitted bit with probability p. Shannon's capacity C(p) = 1 - H(p), with H(p) = -p log2 p - (1 - p) log2 (1 - p). At p = 0.5 the channel is useless; at p = 0 or 1 the channel is noiseless.
- Hook: (placeholder hook, flagged)
- Review verdict: [first-pass] RENDER-NEUTRAL TEXT FIX ONLY
- Flags to address:
...
`playgrounds/msc-y1/MAA-ST-kl-divergence-asymmetry` | MAA-ST | SHIPPED | tier:simple

- What it does / physics / visuals: Two probability densities on a 1D axis. Target P is a bimodal mixture of two Gaussians at +/- sep; approximation Q is a single Gaussian with controllable (mu_q, sigma_q). The playground computes the two directions of KL divergence and shows how their argmins differ qualitatively.
- Hook: (placeholder hook, flagged)
- Review verdict: [first-pass] RENDER-NEUTRAL TEXT FIX ONLY
- Flags to address:
...
`playgrounds/msc-y1/MAA-ST-mutual-information-2d` | MAA-ST | SHIPPED | tier:simple

- What it does / physics / visuals: A static, exact, no-integration example: two correlated Gaussian random variables (X, Y) with covariance Sigma. The joint density p(x, y) is rendered as a heatmap; the marginals p(x) and p(y) are drawn above and beside it. Mutual information I(X; Y) is the area you can carve out of the joint by knowing the marginals; for a Gaussian it admits a closed form -0.5 ln(1 - rho^2).
- Hook: (placeholder hook, flagged)
- Review verdict: [first-pass] RENDER-NEUTRAL TEXT FIX ONLY
- Flags to address:
  - [low] Placeholder hooks in spec.md (needs_hook, needs_paragraph); fill frontmatter fields
...
`playgrounds/msc-y1/MF-AQM-dirac-equation-relativistic-hydrogen` | MF-AQM | SHIPPED | HERO-CANDIDATE | tier:hero
...
`playgrounds/msc-y1/MF-AQM-scattering-theory-differential-cross-section` | MF-AQM | SHIPPED | HERO-CANDIDATE | tier:hero
...
`playgrounds/msc-y1/MF-AQM-second-quantization-bosons-fermions` | MF-AQM | SHIPPED | HERO-CANDIDATE | tier:hero
...
`playgrounds/msc-y1/MF-GR-friedmann-expansion-multicomponent` | MF-GR | SHIPPED | HERO-CANDIDATE | tier:hero
...
`playgrounds/msc-y1/MF-GR-geodesics-curved-spacetime-3d` | MF-GR | SHIPPED | HERO-CANDIDATE | tier:hero
...
`playgrounds/msc-y1/MF-GR-gravitational-wave-detector` | MF-GR | SHIPPED | HERO-CANDIDATE | tier:hero
...
`playgrounds/msc-y1/MF-GR-inflation-quantum-fluctuations` | MF-GR | SHIPPED | HERO-CANDIDATE | tier:hero
...
`playgrounds/msc-y1/MF-PP-feynman-diagram-builder-qed` | MF-PP | SHIPPED | tier:standard
...
`playgrounds/msc-y1/MF-PP-particle-accelerator-betatron` | MF-PP | SHIPPED | tier:standard
...
`playgrounds/msc-y1/MF-PP-standard-model-particle-zoo` | MF-PP | SHIPPED | HERO-CANDIDATE | tier:hero
...
`playgrounds/msc-y1/MF-QFT-casimir-effect-zero-point-energy` | MF-QFT | SHIPPED | HERO-CANDIDATE | tier:hero
...
`playgrounds/msc-y1/MF-QFT-klein-gordon-wavepacket-mass` | MF-QFT | SHIPPED | HERO-CANDIDATE | tier:hero
...
`playgrounds/msc-y1/MF-QFT-symmetry-breaking-mexican-hat` | MF-QFT | SHIPPED | HERO-CANDIDATE | tier:hero
...
`playgrounds/msc-y1/MFM-MP-ct-reconstruction-lab` | MFM-MP | SHIPPED | HERO-CANDIDATE | tier:hero
...
`playgrounds/msc-y1/MFM-MP-monte-carlo-photon-transport` | MFM-MP | SHIPPED | tier:standard
...
`playgrounds/msc-y1/MFM-MP-mri-bloch-equations-k-space` | MFM-MP | SHIPPED | HERO-CANDIDATE | tier:hero
...
`playgrounds/msc-y1/MFM-MP-proton-therapy-bragg-peak` | MFM-MP | SHIPPED | tier:standard
...
`playgrounds/msc-y1/MFM-MP-radiation-dosimetry-detector` | MFM-MP | SHIPPED | tier:standard
