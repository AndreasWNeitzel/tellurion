...
Generated by scripts/make-catalogue.mjs. Total 310 playgrounds: 310 shipped (folder has a .verified marker with a verified-at line), 0 Draft (no .verified, so build-index.mjs gates them out of the gallery), 310 post-build-swept (have DEVNOTES.md), 65 flagged hero_candidate.
...
`playgrounds/msc-y1/MAA-SE-stellar-structure-full-model` | MAA-SE | SHIPPED | swept | tier:hero
