...
window.playground.getState = function () {
  return {
    fields: [
      { key: 'rotation-period-ms', label: 'Rotation period', value: st.rotationPeriod || 1, format: 'float' },
      { key: 'magnetic-field-g', label: 'Magnetic field', value: st.fieldStrength || 1e12, format: 'float' },
      { key: 'beam-angle-degrees', label: 'Beam angle', value: st.beamAngle || 10, format: 'float' }
    ]
  };
};
window.playground.getInvariants = function () {
  // Check that period is physically realistic for pulsars (>1 ms).
  const P = st.rotationPeriod || 1;
  const status = (P > 0.001 && P < 10) ? 'pass' : 'drift';
  return [
    {
      key: 'pulsar-period-bound',
      label: 'Period check (ms)',
      value: P.toFixed(3),
      status: status
    }
  ];
