...
window.playground.getState = function () {
  return {
    fields: [
      { key: 'magnetic-field-strength-g', label: 'Field strength', value: st.fieldStrength || 1e15, format: 'float' },
      { key: 'burst-duration-s', label: 'Burst duration', value: st.burstDuration || 0.1, format: 'float' },
      { key: 'energy-erg', label: 'Burst energy', value: st.energy || 1e40, format: 'float' }
    ]
  };
};
window.playground.getInvariants = function () {
  // Check energy budget: flare energy less than magnetic energy.
  const E = st.energy || 1e40;
  const B = st.fieldStrength || 1e15;
  const Vol = 1e27;  // rough magnetar volume
  const EMagFrac = E / (B * B * Vol);
  const status = EMagFrac < 0.1 ? 'pass' : 'drift';
  return [
    {
      key: 'magnetic-energy-ratio',
      label: 'Energy/mag-energy',
      value: EMagFrac.toExponential(2),
      status: status
    }
  ];
};
