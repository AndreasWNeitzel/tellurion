...
window.playground.getState = function () {
  return {
    fields: [
      { key: 'dimensionless-spin', label: 'Spin parameter a/M', value: st.spin || 0, format: 'float' },
      { key: 'radius-mass-units', label: 'Schwarzschild radius', value: st.schwarzschildRadius || 2, format: 'float' },
      { key: 'ergosphere-ratio', label: 'Ergosphere extent', value: st.ergoRatio || 0, format: 'float' }
    ]
  };
};
window.playground.getInvariants = function () {
  // Check that for Schwarzschild (a=0), Schwarzschild radius = 2M.
  const a = st.spin || 0;
  const status = (a >= 0 && a < 1) ? 'pass' : 'fail';
  return [
    {
      key: 'spin-parameter-bounds',
      label: 'Spin bounds check',
      value: Math.max(0, a).toFixed(3),
      status: status
    }
  ];
};
