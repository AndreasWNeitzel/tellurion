...
function drawReconnectionDiagnostic() {
  ...
  ctx.fillText('reconnection rate vs S', px + 8, py + 14);
...
window.playground.getState = function () {
  ...
  return {
    fields: [
      { key: 'lundquist', label: 'Lundquist number $S$', value: lundquist(p), format: 'sci' },
      { key: 'reconnection-rate', label: 'reconnection rate $M_A$', value: reconnectionRate(p), format: 'float' },
      { key: 'inflow-speed', label: 'inflow speed', value: inflowSpeed(p), format: 'float' },
      { key: 'sheet-width', label: 'current-sheet half-width $\\delta$', value: sheetHalfWidth(p), format: 'float' },
    ],
  };
};
