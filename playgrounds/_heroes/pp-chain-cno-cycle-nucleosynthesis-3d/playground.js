...
window.playground.getState = function () {
  const pp_frac = ppFraction(st.T7);
  const cno_frac = cnoFraction(st.T7);
  return {
    fields: [
      { key: 'temperature', label: 'Core temp (10^7 K)', value: st.T7, format: 'float' },
      { key: 'pp-fraction', label: 'pp-chain fraction', value: pp_frac, format: 'float' },
      { key: 'cno-fraction', label: 'CNO-cycle fraction', value: cno_frac, format: 'float' },
      { key: 'q-value', label: 'Q-value (MeV)', value: Q_HELIUM, format: 'float' }
    ]
  };
};
window.playground.getInvariants = function () {
  const pp_frac = ppFraction(st.T7);
  const cno_frac = cnoFraction(st.T7);
  const frac_sum = pp_frac + cno_frac;
  const err = Math.abs(frac_sum - 1.0);
  return [
    {
      key: 'pp-cno-completeness',
      label: 'pp + CNO = 1',
