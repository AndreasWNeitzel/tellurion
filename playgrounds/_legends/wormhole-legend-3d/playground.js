...
window.playground.getState = function () {
  const rho = exoticEnergyDensity_SI(st.b0);
  const l_m = st.lCam * st.b0;
  const r_l = circumferentialR(l_m, st.b0);
  return {
    fields: [
      { key: 'throat', label: 'Throat radius b0 (m)', value: st.b0, format: 'float' },
      { key: 'exotic-rho', label: 'Exotic density (J/m^3)', value: rho, format: 'float' },
      { key: 'radius-at-l', label: 'r(l) / b0', value: r_l / st.b0, format: 'float' },
      { key: 'mode', label: 'Display mode', value: st.mode, format: undefined }
    ]
  };
};
window.playground.getInvariants = function () {
  const b0_positive = st.b0 > 0;
  return [
    {
      key: 'throat-positive',
      label: 'b0 > 0',
      value: b0_positive ? 'pass' : `b0=${st.b0}`,
      status: b0_positive ? 'pass' : 'drift'
    }
  ];
};
