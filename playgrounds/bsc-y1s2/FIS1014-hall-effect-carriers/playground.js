...
const btnCarrier = document.getElementById('btn-carrier'), vCarrier = document.getElementById('value-carrier');
...
function syncVals() {
  vCarrier.textContent = carrier().label;
...
btnCarrier.addEventListener('click', () => { st.carrier = st.carrier === 'electron' ? 'hole' : 'electron'; syncVals(); });
...
let BAR = null;
function setBar(r) {
  const bw = r.w * 0.55, bh = Math.min(r.h * 0.42, 210);
  const blockH = bh + 24 + 22; // bar + current arrow + readout strip
  const bx = r.x + r.w * 0.055, by = r.y + 28 + Math.max(0, (r.h - 28 - blockH) * 0.5);
  BAR = { x: bx, y: by, w: bw, h: bh };
}
function bx(nx) { return BAR.x + nx * BAR.w; }
function by(ny) { return BAR.y + BAR.h * (0.5 - 0.5 * ny); } // ny in [-1,1], +1 = top

function drawScene(col, r) {
  panel(col, r, 'Lorentz deflection builds the Hall voltage');
  setBar(r);
  const c = carrier(), p = si();
  const VH = VH_mV();
  const topPol = st.B === 0 ? 0 : c.sign * Math.sign(st.B);
  const edgeInt = Math.min(1, Math.abs(restore) * 2.6);

  // magnetic field symbols tiling the bar (dots out of page for B>0, crosses for B<0).
  const fa = Math.min(0.5, Math.abs(st.B) * 0.55);
  if (Math.abs(st.B) > 1e-3) {
    ctx.strokeStyle = `rgba(255,209,102,${fa.toFixed(3)})`; ctx.fillStyle = `rgba(255,209,102,${fa.toFixed(3)})`; ctx.lineWidth = 1.4;
    for (let i = 0; i < 8; i += 1) for (let j = 0; j < 4; j += 1) {
      const X = bx((i + 0.5) / 8), Y = by(1 - (j + 0.5) / 4 * 2); const R = 6;
      ctx.beginPath(); ctx.arc(X, Y, R, 0, 6.28); ctx.stroke();
      if (st.B > 0) { ctx.beginPath(); ctx.arc(X, Y, 1.6, 0, 6.28); ctx.fill(); }
      else { ctx.beginPath(); ctx.moveTo(X - 4, Y - 4); ctx.lineTo(X + 4, Y + 4); ctx.moveTo(X + 4, Y - 4); ctx.lineTo(X - 4, Y + 4); ctx.stroke(); }
    }
  }
  // the bar.
  ctx.fillStyle = col.bar; ctx.fillRect(BAR.x, BAR.y, BAR.w, BAR.h);
  // edge charge bands.
  const band = 9;
  const tCol = topPol > 0 ? col.pos : col.neg, bCol = topPol > 0 ? col.neg : col.pos;
  if (st.B !== 0) {
    ctx.fillStyle = topPol > 0 ? `rgba(255,93,93,${(0.45 * edgeInt).toFixed(3)})` : `rgba(91,140,255,${(0.45 * edgeInt).toFixed(3)})`;
    ctx.fillRect(BAR.x, BAR.y, BAR.w, band);
    ctx.fillStyle = topPol > 0 ? `rgba(91,140,255,${(0.45 * edgeInt).toFixed(3)})` : `rgba(255,93,93,${(0.45 * edgeInt).toFixed(3)})`;
    ctx.fillRect(BAR.x, BAR.y + BAR.h - band, BAR.w, band);
    // +/- signs along the edges.
    ctx.font = fontString(canvas, 'tick', 'mono', 700); ctx.textAlign = 'center'; ctx.textBaseline = 'middle';
    for (let i = 0; i < 7; i += 1) { const X = bx((i + 0.5) / 7);
      ctx.fillStyle = tCol; ctx.fillText(topPol > 0 ? '+' : '−', X, BAR.y + band / 2 + 1);
      ctx.fillStyle = bCol; ctx.fillText(topPol > 0 ? '−' : '+', X, BAR.y + BAR.h - band / 2 - 1); }
  }
  // carriers.
  for (const k of carriers) {
    const X = bx(k.x), Y = by(k.y);
    ctx.fillStyle = c.color; ctx.beginPath(); ctx.arc(X, Y, 2.6, 0, 6.28); ctx.fill();
  }
  // bar outline.
  ctx.strokeStyle = col.barEdge; ctx.lineWidth = 1.5; ctx.strokeRect(BAR.x, BAR.y, BAR.w, BAR.h);

  // current arrow (along the drift axis) below the bar.
  ctx.strokeStyle = col.cur; ctx.fillStyle = col.cur; ctx.lineWidth = 2;
  const ay = BAR.y + BAR.h + 24;
  ctx.beginPath(); ctx.moveTo(BAR.x, ay); ctx.lineTo(BAR.x + BAR.w, ay); ctx.stroke();
  const ah = BAR.x + BAR.w; ctx.beginPath(); ctx.moveTo(ah, ay); ctx.lineTo(ah - 9, ay - 4); ctx.lineTo(ah - 9, ay + 4); ctx.closePath(); ctx.fill();
  ctx.font = fontString(canvas, 'caption', 'mono', 700); ctx.textAlign = 'left'; ctx.textBaseline = 'bottom';
  ctx.fillText(`current I = ${st.I.toFixed(1)} mA`, BAR.x + 4, ay - 6);

  // the Hall field arrow inside the bar near the right edge, pointing from + to - edge.
  if (st.B !== 0 && edgeInt > 0.04) {
    const fx = BAR.x + BAR.w - 24; const y1 = BAR.y + band + 6, y2 = BAR.y + BAR.h - band - 6;
    const fromTop = topPol > 0; // field points away from the + edge
    ctx.strokeStyle = col.field; ctx.fillStyle = col.field; ctx.lineWidth = 2;
    const ya = fromTop ? y1 : y2, yb = fromTop ? y2 : y1;
    ctx.beginPath(); ctx.moveTo(fx, ya); ctx.lineTo(fx, yb); ctx.stroke();
    const dir = Math.sign(yb - ya); ctx.beginPath(); ctx.moveTo(fx, yb); ctx.lineTo(fx - 4, yb - 8 * dir); ctx.lineTo(fx + 4, yb - 8 * dir); ctx.closePath(); ctx.fill();
    ctx.save(); ctx.translate(fx - 11, (y1 + y2) / 2); ctx.rotate(-Math.PI / 2); ctx.textAlign = 'center'; ctx.textBaseline = 'middle'; ctx.font = fontString(canvas, 'tick', 'mono', 700); ctx.fillText('Hall field', 0, 0); ctx.restore();
  }
  // voltmeter beside the bar, wired to the two edges (+ lead on the positive edge).
  const vmW = Math.min(160, r.x + r.w - (BAR.x + BAR.w) - 40 - 14), vmH = 52;
  const vmX = BAR.x + BAR.w + 40, vmY = BAR.y + BAR.h / 2 - vmH / 2;
  // leads.
  const topPos = topPol > 0;
  ctx.strokeStyle = col.muted; ctx.lineWidth = 1.4;
  ctx.beginPath(); ctx.moveTo(BAR.x + BAR.w, BAR.y + 3); ctx.lineTo(vmX + 14, BAR.y + 3); ctx.lineTo(vmX + 14, vmY); ctx.stroke();
  ctx.beginPath(); ctx.moveTo(BAR.x + BAR.w, BAR.y + BAR.h - 3); ctx.lineTo(vmX + vmW - 14, BAR.y + BAR.h - 3); ctx.lineTo(vmX + vmW - 14, vmY + vmH); ctx.stroke();
  ctx.font = fontString(canvas, 'tick', 'mono', 700); ctx.textAlign = 'center'; ctx.textBaseline = 'middle';
  if (st.B !== 0) { ctx.fillStyle = topPos ? col.pos : col.neg; ctx.fillText(topPos ? '+' : '−', vmX + 14, vmY - 8); ctx.fillStyle = topPos ? col.neg : col.pos; ctx.fillText(topPos ? '−' : '+', vmX + vmW - 14, vmY + vmH + 8); }
  ...
  ctx.font = fontString(canvas, 'tick', 'mono'); ctx.fillStyle = col.muted; ctx.fillText('Hall V_H', vmX + vmW / 2, vmY + 13);
  ctx.font = fontString(canvas, 'heading', 'mono', 700); ctx.fillStyle = VH > 0 ? col.pos : VH < 0 ? col.neg : col.muted;
  ctx.fillText(`${VH >= 0 ? '+' : ''}${VH.toFixed(2)} mV`, vmX + vmW / 2, vmY + 34);
