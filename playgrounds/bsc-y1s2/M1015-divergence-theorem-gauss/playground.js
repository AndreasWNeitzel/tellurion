// The divergence theorem in the plane: the outward flux through a closed curve
// equals the area integral of the divergence inside it. The scene shows the
// vector field, a divergence heatmap, and a draggable, resizable circle whose
// boundary is coloured by the outflow F.n; the two numbers (flux and area
// integral of div) are computed live and agree. Canvas2D only.
//
// Reference: Stewart, Calculus, 8e, Sec. 16.9.
...
const EXT = 2.5;                                   // world half-extent
const st = { field: 'source', cx: 0.5, cy: 0.3, R: 1.0 };
...
btnReset.addEventListener('click', () => { st.field = 'source'; st.cx = 0.5; st.cy = 0.3; st.R = 1.0; sR.value = '1'; syncVals(); render(); });

function colors() {
  return { bg: '#06070c', panel: '#0a0c12', fg: '#e8e8e8', muted: '#9aa0a6', border: 'rgba(255,255,255,0.12)', grid: 'rgba(255,255,255,0.09)', arrow: 'rgba(210,218,235,0.6)', curve: '#ffd166', out: '#ef5466', in: '#5b8def', flux: '#ef5466', area: '#67d98c' };
}
...
function drawScene(col, r) {
  panel(col, r, 'Outward flux through the curve equals the area integral of div F inside');
  ...
  const X = (wx) => ox + (wx + EXT) / (2 * EXT) * size;
  const Y = (wy) => oy + (EXT - wy) / (2 * EXT) * size;
  const wX = (sx) => (sx - ox) / size * (2 * EXT) - EXT;
  const wY = (sy) => EXT - (sy - oy) / size * (2 * EXT);
  SCN = { ox, oy, size, X, Y, wX, wY };
  const f = fld();

  // divergence heatmap (offscreen, rdbu).
  const NH = 90;
  ...
  // singular point (point source).
  if (f.singular) { ctx.fillStyle = enclosesOrigin(st.cx, st.cy, st.R) ? col.out : col.muted; ctx.beginPath(); ctx.arc(X(0), Y(0), 5, 0, 6.28); ctx.fill(); }

  // the closed curve, boundary coloured by F.n.
  const Rpix = st.R / (2 * EXT) * size; const n = 160;
  for (let i = 0; i < n; i += 1) {
    const th = i / n * 2 * Math.PI, c = Math.cos(th), s = Math.sin(th);
    const [fx, fy] = f.F(st.cx + st.R * c, st.cy + st.R * s); const fn = fx * c + fy * s;
    ctx.strokeStyle = fn >= 0 ? col.out : col.in; ctx.globalAlpha = Math.min(1, 0.35 + Math.abs(fn) * 0.5); ctx.lineWidth = 3;
  ...
  // readout strip.
  const flux = fluxCircle(f, st.cx, st.cy, st.R);
  const area = f.singular ? (enclosesOrigin(st.cx, st.cy, st.R) ? 2 * Math.PI : 0) : divIntegralCircle(f, st.cx, st.cy, st.R);
  const items = [[`flux ${flux.toFixed(3)}`, col.flux], [`integral div ${area.toFixed(3)}`, col.area], [f.singular ? (enclosesOrigin(st.cx, st.cy, st.R) ? 'source enclosed' : 'source outside') : `|flux - integral| ${Math.abs(flux - area).toExponential(0)}`, col.curve]];
  ...
  ctx.fillText('drag to move the circle; red boundary = outflow, blue = inflow', r.x + r.w / 2, r.y + r.h - 10);
}

function drawDiag(col, r) {
  panel(col, r, 'Flux and the area integral of div F versus the circle radius: they coincide');
  const inner = { x: r.x + 46, y: r.y + 26, w: r.w - 46 - 16, h: r.h - 26 - 30 };
  const f = fld(); const Rmax = 2.2;
  const xs = [];
  for (let i = 1; i <= 60; i += 1) { const R = Rmax * i / 60; const flux = fluxCircle(f, st.cx, st.cy, R); const area = f.singular ? (enclosesOrigin(st.cx, st.cy, R) ? 2 * Math.PI : 0) : divIntegralCircle(f, st.cx, st.cy, R, 90); xs.push({ R, flux, area }); }
  let lo = 0, hi = 0.1; for (const p of xs) { lo = Math.min(lo, p.flux, p.area); hi = Math.max(hi, p.flux, p.area); }
  ...
  // area integral (green, thick) then flux (red dashed on top) so coincidence shows.
  ...
  ctx.strokeStyle = col.flux; ctx.lineWidth = 1.8; ctx.setLineDash([5, 4]); ctx.beginPath(); xs.forEach((p, i) => { const X = xOf(p.R), Y = yOf(p.flux); i ? ctx.lineTo(X, Y) : ctx.moveTo(X, Y); }); ctx.stroke(); ctx.setLineDash([]);
  // current R marker.
  ...
  ctx.fillStyle = col.area; ctx.fillText('integral of div', inner.x + 6, inner.y + 4);
  ctx.fillStyle = col.flux; ctx.fillText('flux (dashed)', inner.x + 6, inner.y + 18);
...
// drag the circle centre within the scene.
let dragging = false;
...
window.playground.getState = function () {
  const f = fld(); const flux = fluxCircle(f, st.cx, st.cy, st.R);
  const area = f.singular ? (enclosesOrigin(st.cx, st.cy, st.R) ? 2 * Math.PI : 0) : divIntegralCircle(f, st.cx, st.cy, st.R);
  return { fields: [
    { key: 'field', label: 'field', value: f.label, format: 'text' },
    { key: 'R', label: 'circle radius', value: st.R, format: 'float' },
    { key: 'flux', label: 'outward flux', value: flux, format: 'float' },
    { key: 'area', label: 'integral of div', value: area, format: 'float' },
  ] };
};
window.playground.getInvariants = function () {
  ...
  if (f.singular) {
    ...
    return [{ key: 'gauss', label: inside ? 'flux = 2 pi (source enclosed)' : 'flux = 0 (source outside)', value: flux.toFixed(3), status: Math.abs(flux - (inside ? 2 * Math.PI : 0)) < 0.05 ? 'pass' : 'drift' }];
  }
  const area = divIntegralCircle(f, st.cx, st.cy, st.R);
  const err = Math.abs(flux - area) / (Math.abs(area) + 0.2);
  return [{ key: 'divthm', label: 'flux = integral of div F', value: err.toExponential(1), status: err < 0.02 ? 'pass' : 'drift' }];
};
...
function __autoOrbit(now) {
  ...
  if (!DETERMINISTIC && typeof dragging !== 'undefined' && !dragging) {
    __aphase += dt * 0.45;
    st.cx = 0 + 1.4 * Math.cos(__aphase);
    st.cy = 0 + 1.1 * Math.sin(__aphase * 1.3);
    render();
  }
