...
// State reports the mirror reflectance, the finesse and the fringe
// FWHM. The invariant cross-checks the FWHM against the Airy
// transmission: evaluated at half the FWHM phase, the transmission
// must equal exactly 1/2 (the definition of full-width-half-max).
window.playground = window.playground || {};
window.playground.getState = function () {
  return {
    fields: [
      { key: 'reflectance', label: 'mirror reflectance R', value: R, format: 'float' },
      { key: 'finesse', label: 'finesse F*', value: finesse(R), format: 'float' },
      { key: 'fwhm', label: 'fringe FWHM (rad)', value: fwhmPhi(R), format: 'float' },
    ],
  };
};
window.playground.getInvariants = function () {
  const halfMax = transmission(fwhmPhi(R) / 2, R);
  const drift = Math.abs(halfMax - 0.5);
  return [{
    key: 'fwhm',
    label: 'transmission = 1/2 at the half-maximum phase (FWHM consistency)',
