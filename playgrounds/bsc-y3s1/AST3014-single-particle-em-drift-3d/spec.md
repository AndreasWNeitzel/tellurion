...
one_paragraph: 'An interactive single-particle picture of charged-particle motion in static electric and magnetic fields, m dv/dt = q(E + v x B), integrated with the Boris pusher (a time-reversible leapfrog that conserves speed exactly in a pure magnetic field). Presets show the cyclotron helix (omega_c = qB/m), the E x B drift (v_d = E x B / B^2, the same for every charge and mass), the grad-B and curvature drifts, and a magnetic mirror that reflects the particle while the adiabatic invariant mu = m v_perp^2 / 2B is conserved. The 3D orbit is drawn with a Canvas2D orthographic projection (deterministic and gate-robust; honestly a projection of the real 3D trajectory, not a WebGL scene). The headline readouts are |v| (constant to 1e-9 in pure B, the Boris property) and mu. Physics is the gate-tested shared sim.js.'
