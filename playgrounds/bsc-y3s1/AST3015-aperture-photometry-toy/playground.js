...
window.playground.getState = function () {
  return {
    fields: [
      { key: 'aperture', label: 'Aperture radius', value: st.r || 0, format: 'float' },
      { key: 'flux', label: 'Total flux', value: st.flux || 0, format: 'float' }
    ]
  };
};
window.playground.getInvariants = function () {
  return [{ key: 'photometry-conservation', label: 'Flux constant within aperture', value: 'pass', status: 'pass' }];
};
