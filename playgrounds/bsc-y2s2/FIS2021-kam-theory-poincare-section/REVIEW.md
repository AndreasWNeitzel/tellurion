# REVIEW - FIS2021-kam-theory-poincare-section (deep audit; supersedes any earlier pass)
...
- Figcaption or user-visible text exposes raw bibliography key in backticks: `lichtenberg-lieberman`. This is a rendering artifact and must be removed. Change all instances of `(lichtenberg-lieberman)` or text containing backtick-wrapped bibkey to plain text (remove backticks and the key itself, or replace with the full citation).
...
- [ ] **IMMEDIATE:** Remove all backtick-wrapped bibliography keys (`lichtenberg-lieberman`) from user-facing text (figcaptions, descriptions, captions).
