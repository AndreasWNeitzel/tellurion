...
window.playground.getState = function () {
  ...
  return {
    fields: [
      { key: 'detector-strength', label: 'Which-path detector', value: st.detector, format: 'float' },
      { key: 'slit-separation', label: 'Slit separation (um)', value: 1e4 * p.d, format: 'float' },
      { key: 'wavelength', label: 'Wavelength (nm)', value: p.lambda * 1e9, format: 'float' },
      { key: 'visibility', label: 'Fringe visibility V', value: vis, format: 'float' },
    ],
  };
};
window.playground.getInvariants = function () {
  ...
  const vis = visibility(p);
  const dy = fringeSpacing(p);
  const detectorOk = st.detector >= 0 && st.detector <= 1;
  const visOk = vis >= 0 && vis <= 1 && (st.detector === 0 ? vis > 0.9 : vis < 0.1);
  return [
    {
      key: 'visibility-detector-coupling',
      label: 'Visibility vanishes with detector (Born rule)',
      value: (detectorOk && visOk) ? 'pass' : 'drift',
      status: (detectorOk && visOk) ? 'pass' : 'drift',
    },
  ];
};
