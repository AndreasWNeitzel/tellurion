...
const st = { detector: 0.0, d: 1.0, lambda: 1.0, count: 0, running: 1 };
const BASE = { lambda: 5e-7, L: 2, a: 2e-5 };
function gammaC() { return 1 - st.detector; }
function P() { return { lambda: BASE.lambda * st.lambda, L: BASE.L, d: 1e-4 * st.d, a: BASE.a, gamma: gammaC() }; }
...
function render() {
  ...
  const slitGap = (st.d * 1e-4) / Y * (SCH / 2) * 0.5 + 16;
  ctx.fillStyle = '#3a3f4b'; ctx.fillRect(barX - 4, SCY, 8, SCH);
  ctx.clearRect(barX - 4, srcY - slitGap - 5, 8, 10);
  ctx.clearRect(barX - 4, srcY + slitGap - 5, 8, 10);
  ctx.fillStyle = '#0a0c12'; ctx.fillRect(barX - 4, srcY - slitGap - 5, 8, 10); ctx.fillRect(barX - 4, srcY + slitGap - 5, 8, 10);
...
bReset.addEventListener('click', () => {
  Object.assign(st, { detector: 0.0, d: 1.0, lambda: 1.0, count: 0, running: 1 });
  cG.inp.value = '0'; cG.val.textContent = 'off'; cD.inp.value = '1'; cD.val.textContent = '1.00x'; cL.inp.value = '1'; cL.val.textContent = '1.00x';
