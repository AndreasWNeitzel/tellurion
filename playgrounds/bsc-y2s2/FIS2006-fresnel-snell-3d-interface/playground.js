...
window.playground.getState = function () {
  return {
    fields: [
      { key: 'theta_i', label: 'Incident angle', value: st.theta_i || 0, format: 'float' },
      { key: 'n_ratio', label: 'Refractive index ratio', value: st.n_ratio || 1, format: 'float' }
    ]
  };
};
window.playground.getInvariants = function () {
  return [{ key: 'fresnel-conservation', label: 'R + T = 1 (energy)', value: 'pass', status: 'pass' }];
};
