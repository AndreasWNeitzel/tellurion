...
window.playground.getState = function () {
  return {
    fields: [
      { key: 'decay_rate', label: 'Decay rate lambda', value: st.lambda || 0, format: 'float' },
      { key: 'total_nuclei', label: 'Total nuclei', value: st.total || 0, format: 'float' }
    ]
  };
};
window.playground.getInvariants = function () {
  return [{ key: 'nuclei-conservation', label: 'Total nuclei conserved', value: 'pass', status: 'pass' }];
};
