...
window.playground.getState = function () {
  return {
    fields: [
      { key: 'param-a', label: 'Parameter a', value: state.params.a, format: 'float' },
      { key: 'param-b', label: 'Parameter b', value: state.params.b, format: 'float' },
      { key: 'param-c', label: 'Parameter c', value: state.params.c, format: 'float' },
      { key: 'speed', label: 'Integration speed', value: state.speed, format: 'float' },
      { key: 'max-lyapunov', label: 'Max Lyapunov exponent', value: maxLyapunov(state.rossler, RESCALE_EVERY), format: 'float' }
    ]
  };
};
window.playground.getInvariants = function () {
  if (!state.rossler || !state.rossler.inst) {
    return [{ key: 'state-init', label: 'Initializing', value: 'pending', status: 'pending' }];
  }
  const ly = maxLyapunov(state.rossler, RESCALE_EVERY);
  const pos = state.rossler.inst.state;
  const norm = pos ? Math.sqrt(pos[0] * pos[0] + pos[1] * pos[1] + pos[2] * pos[2]) : 0;
  return [
    {
      key: 'chaos-signature',
      label: 'Positive Lyapunov (chaos)',
      value: ly > 0 ? 'yes' : 'no',
      status: ly > 0.05 ? 'pass' : 'pending'
    },
    {
      key: 'attractor-bounded',
      label: 'Trajectory bounded (norm < 50)',
      value: norm.toExponential(2),
      status: norm < 50 ? 'pass' : 'drift'
    }
  ];
};
