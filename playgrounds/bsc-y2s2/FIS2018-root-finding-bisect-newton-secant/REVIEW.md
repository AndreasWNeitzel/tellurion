# REVIEW - FIS2018-root-finding-bisect-newton-secant (deep audit; supersedes any earlier pass)
...
- Figcaption or user-visible text exposes raw bibliography key in backticks: `burdenfaires`. This is a rendering artifact and must be removed. Change all instances of `(burdenfaires)` or text containing backtick-wrapped bibkey to plain text (remove backticks and the key itself, or replace with the full citation).
...
- [ ] **IMMEDIATE:** Remove all backtick-wrapped bibliography keys (`burdenfaires`) from user-facing text (figcaptions, descriptions, captions).
