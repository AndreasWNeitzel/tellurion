# REVIEW - FIS2021-fitzhugh-nagumo-excitable (deep audit; supersedes any earlier pass)
...
- Figcaption or user-visible text exposes raw bibliography key in backticks: `fitzhugh-nagumo1961`. This is a rendering artifact and must be removed. Change all instances of `(fitzhugh-nagumo1961)` or text containing backtick-wrapped bibkey to plain text (remove backticks and the key itself, or replace with the full citation).
...
- [ ] **IMMEDIATE:** Remove all backtick-wrapped bibliography keys (`fitzhugh-nagumo1961`) from user-facing text (figcaptions, descriptions, captions).
