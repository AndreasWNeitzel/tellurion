# REVIEW - FIS2018-gauss-quadrature-vs-trapezoid (deep audit; supersedes any earlier pass)
...
- Figcaption or user-visible text exposes raw bibliography key in backticks: `trefethen-spectral`. This is a rendering artifact and must be removed. Change all instances of `(trefethen-spectral)` or text containing backtick-wrapped bibkey to plain text (remove backticks and the key itself, or replace with the full citation).
...
- [ ] **IMMEDIATE:** Remove all backtick-wrapped bibliography keys (`trefethen-spectral`) from user-facing text (figcaptions, descriptions, captions).
