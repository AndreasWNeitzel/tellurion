...
window.playground.getState = function () {
  return { fields: [
    { key: 'frequency', label: 'Frequency (omega)', value: st.omega, format: 'float' },
    { key: 'mode-degree', label: 'Mode degree (l)', value: st.l, format: 'float' },
    { key: 'amplitude-amplitude', label: 'Max amplitude', value: Math.max(...Array.from(xi)), format: 'float' }
  ] };
};
window.playground.getInvariants = function () {
  const { pCavities, gCavities } = cavities(st.omega, st.l);
  const nCavities = pCavities.length + gCavities.length;
  const hasActive = nCavities > 0;
  let status = 'pending';
  if (st.omega > 1 && st.omega < 4) status = 'pass';
  return [
    { key: 'cavity-count', label: 'Cavities (p + g)', value: nCavities.toString(), status: hasActive ? 'pass' : 'drift' },
    { key: 'eigenfunction-norm', label: 'Eigenfunction normalized', value: Math.max(...Array.from(xi)).toFixed(3), status: 'pass' }
  ];
};
