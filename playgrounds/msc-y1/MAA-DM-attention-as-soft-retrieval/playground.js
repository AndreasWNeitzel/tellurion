...
window.playground.getState = function () {
  const attn = attention(state.query, state.keys, state.values, state.tau);
  return {
    fields: [
      { key: 'tau', label: 'temperature tau', value: state.tau, format: 'float' },
      { key: 'query-x', label: 'query position x', value: state.query[0], format: 'float' },
      { key: 'query-y', label: 'query position y', value: state.query[1], format: 'float' },
      { key: 'entropy', label: 'attention weight entropy (nats)', value: entropy(attn.weights), format: 'float' }
    ]
  };
};
window.playground.getInvariants = function () {
  const inv = [];
  const attn = attention(state.query, state.keys, state.values, state.tau);
  // Softmax normalization: weights must sum to 1.0
  const wsum = attn.weights.reduce((a, b) => a + b, 0);
  inv.push({
    key: 'weight-normalization',
    label: 'attention weights sum to 1.0',
    value: wsum.toFixed(6),
    status: Math.abs(wsum - 1.0) < 1e-10 ? 'pass' : 'drift'
  });
  // Entropy bounds: H must be in [0, log(N_KEYS)] = [0, log(6)]
  const H = entropy(attn.weights);
  const maxH = Math.log(N_KEYS);
  inv.push({
    key: 'entropy-bounds',
    label: 'entropy 0 <= H <= log(N)',
    value: H.toFixed(3),
    status: H >= 0 && H <= maxH + 1e-10 ? 'pass' : 'drift'
  });
  return inv;
};
