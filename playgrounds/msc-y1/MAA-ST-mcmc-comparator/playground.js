...
window.playground.getState = function () {
  const fields = [
    { key: 'target', label: 'target distribution', value: state.targetName, format: undefined },
  ];
  if (state.chains && state.chains.length > 0) {
    fields.push({ key: 'acceptance', label: 'acceptance rate (%)', value: (100 * state.chains[0].acceptance).toFixed(1), format: 'float' });
  }
  return { fields };
};
window.playground.getInvariants = function () {
  if (!state.chains || state.chains.length === 0) {
    return [{ key: 'chains-init', label: 'chains initialized', value: 'pending', status: 'pending' }];
  }
  const accs = state.chains.map(c => c.acceptance);
  return [
    {
      key: 'sampling-active',
      label: 'MCMC chains sampling',
      value: `acc rates: ${accs.map(a => (100 * a).toFixed(0) + '%').join(', ')}`,
      status: 'pass',
    },
  ];
};
