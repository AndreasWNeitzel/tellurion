...
window.playground.getState = function () {
  return {
    fields: [
      { key: 'chirp-mass', label: 'chirp mass $M_c$ ($M_\\odot$)', value: st.McSun ?? 0, format: 'float' },
      { key: 'recovered-mass', label: 'recovered $M_c$ ($M_\\odot$)', value: st.recMc ?? 0, format: 'float' },
      { key: 'strain', label: 'peak strain $h$', value: st.wf ? Math.max(...st.wf.amp) : 0, format: 'sci' },
      { key: 'snr', label: 'matched-filter SNR peak', value: st.mf ? st.mf.peak : 0, format: 'sci' },
    ],
  };
};
