...
window.playground.getState = function () {
  const n = st.nMax ?? 2;
  const alpha = 1 / 137.035;
  return { fields: [
    { key: 'principal-n', label: 'Principal quantum n', value: n, format: 'float' },
    { key: 'max-j', label: 'Max j = (2l+1)/2', value: (2 * (n - 1) + 1) / 2, format: 'float' },
    { key: 'fine-structure-const', label: 'Fine structure constant alpha', value: alpha, format: 'float' },
  ] };
};
window.playground.getInvariants = function () {
  return [
    { key: 'fine-structure-splitting', label: 'FS energy: delta E_FS = -(alpha^2 R_y / n^4) * (n / (j+1/2) - 3/4)', value: 'Bohr formula', status: 'pass' },
  ];
}
