...
one_paragraph: 'An interactive closed-form view of steady incompressible flow through a variable-area pipe (a Venturi). Mass conservation fixes Q = A(x) v(x) = const, so the speed rises where the pipe narrows; Bernoulli p + 1/2 rho v^2 = const (horizontal pipe) then drops the static pressure there. The Canvas2D scene draws the pipe, tracer particles that accelerate through the throat, piezometer columns whose height tracks the static pressure (tall in the wide sections, a stub at the throat), and a thin-airfoil lift cartoon driven by the same principle. The headline readouts are the two invariants, the relative spread of the Bernoulli constant and of the flux A v along the pipe, both held below 1e-3 (machine precision in practice) because the physics is exact algebra in shared sim.js, not a numerical solver. Controls set the throat ratio, the flow rate, and the density; the run is deterministic.'
