...
window.playground.getState = function () {
  const fields = [
    { key: 'potential-strength', label: 'potential strength P', value: parseFloat(state.P.toFixed(2)), format: 'float' },
    { key: 'energy-max', label: 'energy max eMax', value: state.eMax, format: 'float' },
  ];
  return { fields };
};
window.playground.getInvariants = function () {
  const inv = [];
  inv.push({ key: 'p-positive', label: 'P > 0', value: state.P > 0 ? 'pass' : 'drift', status: state.P > 0 ? 'pass' : 'drift' });
  inv.push({ key: 'emax-positive', label: 'eMax > 0', value: state.eMax > 0 ? 'pass' : 'drift', status: state.eMax > 0 ? 'pass' : 'drift' });
  return inv;
};
