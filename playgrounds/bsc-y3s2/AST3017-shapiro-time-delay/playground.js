...
window.playground.getState = function () {
  ...
  return {
    fields: [
      { key: 'impact-parameter', label: 'Impact parameter b (M)', value: state.b, format: 'float' },
      { key: 'receiver-distance', label: 'Emitter-Receiver dist (M)', value: state.r, format: 'float' },
      { key: 'shapiro-delay', label: 'Shapiro delay (M)', value: delay, format: 'float' },
      { key: 'race-time', label: 'Race time (M)', value: state.tau, format: 'float' }
    ]
  };
};
window.playground.getInvariants = function () {
  const delay = shapiroDelay(state.r, state.r, state.b);
  const delayNonNeg = delay >= 0;
  const bPositive = state.b > 0;
  const rPositive = state.r > 0;
  return [
    {
      key: 'delay-nonnegativity',
      label: 'Shapiro delay >= 0',
      value: delayNonNeg ? 'pass' : 'fail',
      status: delayNonNeg ? 'pass' : 'drift'
    },
    {
      key: 'parameter-positivity',
      label: 'Impact param and distance positive',
      value: bPositive && rPositive ? 'pass' : 'fail',
      status: bPositive && rPositive ? 'pass' : 'drift'
    }
  ];
};
