...
window.playground.getState = function () {
  ...
  return {
    fields: [
      { key: 'orbital', label: 'orbital label', value: orb.label, format: undefined },
      { key: 'quantum-n', label: 'principal quantum number', value: orb.n, format: 'float' },
      { key: 'quantum-l', label: 'orbital angular momentum', value: orb.l, format: 'float' },
      { key: 'quantum-m', label: 'magnetic quantum number', value: orb.m, format: 'float' },
    ],
  };
};
window.playground.getInvariants = function () {
  const orb = ORBITALS[state.idx];
  const nValid = orb.n >= 1;
  const lValid = orb.l >= 0 && orb.l < orb.n;
  const mValid = Math.abs(orb.m) <= orb.l;
  const allValid = nValid && lValid && mValid;
  return [
    {
      key: 'quantum-number-rules',
      label: 'quantum number constraints hold',
      value: allValid ? 'valid' : `n=${orb.n} l=${orb.l} m=${orb.m}`,
      status: allValid ? 'pass' : 'drift',
    },
  ];
};
