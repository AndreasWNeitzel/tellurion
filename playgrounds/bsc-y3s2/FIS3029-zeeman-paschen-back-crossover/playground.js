...
window.playground.getState = function () {
  return { fields: [
    { key: 'magnetic-field', label: 'Magnetic field (Tesla)', value: st.B, format: 'float' },
    { key: 'principal-n', label: 'Principal quantum number n', value: st.n ?? 2, format: 'float' },
    { key: 'orbital-l', label: 'Orbital angular momentum l', value: st.l ?? 1, format: 'float' },
  ] };
};
window.playground.getInvariants = function () {
  return [
    { key: 'zeeman-paschen-back', label: 'Zeeman (weak B) to Paschen-Back (strong B) crossover', value: 'B-dependent', status: 'drift' },
  ];
}
