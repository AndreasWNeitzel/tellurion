...
## Live-fix 2026-05-19
User: "needs to be zoomed out so we can see the curve being traced out; the 3D object could be more beautifully rendered". My earlier promotion set camera radius 5.8, which cropped the invariable-plane disk (r=3.4) and the polhode/herpolhode. Set radius 9.6 (maxRadius 20) so the whole Poinsot construction - gridded inertia ellipsoid with shading + rim, translucent invariable-plane disk, polhode on the body, herpolhode loop in the plane around L - is fully visible. sim.js/invariants byte-identical (5/5); verified live the traced curve is no longer cut off and the body tumbles (Dzhanibekov, E and |L| conserved).
