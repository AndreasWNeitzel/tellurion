...
window.playground.getState = function () {
  ...
  return {
    fields: [
      ...
      { key: 'gamma', label: 'Damping', value: state.gamma, format: 'float' },
...
window.playground.getInvariants = function () {
  const q = qualityFactor(state.gamma);
  const q_positive = q > 0;
  return [
    {
      key: 'quality-positive',
      label: 'Q > 0',
      value: q_positive ? 'pass' : `Q=${q.toFixed(3)}`,
      status: q_positive ? 'pass' : 'drift'
    }
  ];
};
