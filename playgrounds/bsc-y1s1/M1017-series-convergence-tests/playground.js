...
window.playground.getState = function () {
  ...
  const Sn = partialSum(st.name, st.N);
  const rho = ratioTest(st.name, st.N);
  const nth = rootTest(st.name, st.N);
  return {
    fields: [
      { key: 'series', label: 'series', value: S.label.substring(0, 20), format: undefined },
      { key: 'n-terms', label: 'N (terms)', value: st.N, format: 'float' },
      { key: 'partial-sum', label: 'S_N (sum)', value: Sn, format: 'float' },
      { key: 'ratio-rho', label: 'ratio rho', value: rho, format: 'float' }
    ]
  };
};
window.playground.getInvariants = function () {
  ...
  const rho = ratioTest(st.name, st.N);
  const nth = rootTest(st.name, st.N);
  const isLimitFinite = Number.isFinite(S.limit);
  if (!isLimitFinite) {
    return [{ key: 'divergent', label: 'series diverges', value: 'expected', status: 'pass' }];
  }
  const error = Math.abs(Sn - S.limit);
  const isConverging = error < 0.01 && st.N > 50;
  return [
    {
      key: 'convergence',
      label: 'approaches limit (error)',
      value: error.toExponential(2),
      status: isConverging ? 'pass' : 'drift'
    }
  ];
};
