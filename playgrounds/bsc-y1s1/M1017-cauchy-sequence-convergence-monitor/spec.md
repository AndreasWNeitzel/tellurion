...
one_paragraph: "A sequence is Cauchy if, for every tolerance epsilon, there is an index N0 beyond which all terms lie within epsilon of each other; for real numbers this is exactly convergence. The playground plots a sequence on a log index axis with a sliding tail window from N0, and draws the tail width (the largest gap between any two terms past N0) as a strip measured against the epsilon tolerance. Convergent examples (a geometric sequence, the Leibniz partial sums for arctan, the zeta(2) sum) show the strip collapse toward zero as N0 slides out; the harmonic sum is the counterexample whose tail width never falls below about ln 10, so no N0 ever fits a small epsilon and it does not converge. The diagnostic plots the tail width against N0. The point is operational: convergence is not a vague approach but a precise promise. Name your epsilon and the tail must shrink below it."
