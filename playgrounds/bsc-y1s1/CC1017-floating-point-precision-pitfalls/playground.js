...
// Vertical 4:5 hero for accumulating floating-point clock drift. Top region: a
// fast object on a track and the prediction gate an automated tracker opens for
// it, offset along the track by the accumulated clock-drift error; when the
// offset exceeds the gate's catch radius the object is missed. Bottom region:
// gate displacement versus uptime against the catch radius.
//
// Reference: Goldberg, ACM Comput. Surv. 23(1), 1991 (`goldberg1991`).

import {
  clockError, gateDisplacementMeters, ERR_PER_TICK_S,
} from './sim.js';
...
const GATE_M = 250;            // catch-gate half-width (m): beyond this the object is missed
const CENTER_M = 60;          // centered-catch radius (m): a clean, centered grab
const PXM = 0.14;             // pixels per metre for the offset
const HOURS_MAX = 105;        // auto-age sweep top
const AGE_RATE = 6.0;         // hours of uptime per real second while playing
let running = !DETERMINISTIC;
let p = 0;                    // pass progress 0..1 (loops)
let upH = 18;                 // continuously-aged uptime (hours)

const hours = () => upH;
const speed = () => parseFloat(sliderSpeed.value);
const patched = () => toggleFix.checked;
const disp = () => Math.abs(gateDisplacementMeters(hours(), speed(), patched()));
// Graded outcome (not a single binary flip): a centered catch within the inner
// radius, an edge catch while the gate still covers the object, then a miss once
// the drift exceeds the gate. Catch confidence falls smoothly.
const confidence = () => Math.exp(-((disp() / GATE_M) ** 2));
function outcome() {
  const d = disp();
  if (d <= CENTER_M) return 'catch';
  if (d <= GATE_M) return 'edge';
  return 'miss';
}
...
function drawScene(col, r) {
  panel(col, r, 'Where the tracker aims vs where the object actually is');
  ...
  const centerPx = CENTER_M * PXM;
  const out = outcome();

  // Track: the object crosses from upper-right toward the catcher at bottom.
  ...
  const caught = (out === 'catch') && p >= P_INT;     // object grabbed at the gate
  const mover = along(caught ? P_INT : p);
  const intTrue = along(P_INT);
  // The predicted point (gate centre): the object's true position at the catch
  // instant, shifted back along the track by the accumulated clock drift,
  // because the tracker believes the object is `d` metres behind where it is.
  const gate = { x: intTrue.x - ux * dPx, y: intTrue.y - uy * dPx };

  ctx.save(); clipTo(ctx, draw);

  // Track baseline + catcher station.
  ...
  ctx.fillText('catcher', site.x, site.y + 13);
  ...
  // Catch gate, centred on the predicted point. Colour grades with the outcome.
  const gcol = out === 'miss' ? col.mover : (out === 'catch' ? col.ok : col.accent);
  ...
  ctx.fillText('catch gate', gate.x, gate.y - gateHalfPx - 3);

  // Drift offset arrow from the predicted point to the true object.
  if (dPx > 3 && !caught) {
    ctx.strokeStyle = col.accent; ctx.lineWidth = 1.4;
    ctx.beginPath(); ctx.moveTo(gate.x, gate.y); ctx.lineTo(mover.x, mover.y); ctx.stroke();
    ctx.fillStyle = col.accent; ctx.font = fontString(canvas, 'tick', 'mono', 700);
    ctx.textAlign = 'left'; ctx.textBaseline = 'middle';
    ctx.fillText(`${d.toFixed(0)} m off`, (gate.x + mover.x) / 2 + 6, (gate.y + mover.y) / 2 - 6);
  }

  // The catcher reaches from its station toward the predicted point, but only
  // while the gate still covers the object. Past the gate the object is missed
  // and the catcher never closes.
  if (out !== 'miss' && p >= P_LAUNCH) {
  ...
  // The object, unless caught.
  if (!caught) {
    ctx.fillStyle = col.mover; ctx.beginPath(); ctx.arc(mover.x, mover.y, 6, 0, 2 * Math.PI); ctx.fill();
  ...
  if (out === 'catch' && caught) {
    ...
    ctx.textAlign = 'center'; ctx.textBaseline = 'middle'; ctx.fillText('CAUGHT', gate.x, gate.y + gateHalfPx + 16);
  } else if (out !== 'catch' && p > 0.985) {
    ctx.strokeStyle = col.mover; ctx.lineWidth = 2; ctx.beginPath(); ctx.arc(site.x, site.y, 11, 0, 2 * Math.PI); ctx.stroke();
    ctx.fillStyle = col.mover; ctx.font = fontString(canvas, 'caption', 'mono', 700); ctx.textAlign = 'center'; ctx.textBaseline = 'bottom';
    ctx.fillText('MISSED', site.x, site.y - 16);
  }

  // Continuous catch-confidence bar (top-left), so the slider's effect is a
  ...
  ctx.fillStyle = conf > 0.5 ? col.ok : (conf > 0.15 ? col.accent : col.mover);
  ctx.fillRect(bx, by, bw * conf, bh);
  ctx.fillStyle = col.muted; ctx.font = fontString(canvas, 'tick', 'mono');
  ctx.textAlign = 'left'; ctx.textBaseline = 'bottom';
  ctx.fillText(`catch confidence ${(conf * 100).toFixed(0)}%`, bx, by - 2);

  ctx.restore();

  // Readout strip.
  const tErr = clockError(hours(), patched());
  const ry = r.y + r.h - stripH / 2 + 1;
  const label = out === 'catch' ? 'CAUGHT' : (out === 'edge' ? 'EDGE CATCH' : 'MISSED');
  const lcol = out === 'catch' ? col.ok : (out === 'edge' ? col.accent : col.mover);
  const items = [
...
function drawDiagnostic(col, r) {
  ...
  const dMax = Math.max(GATE_M * 1.4, gateDisplacementMeters(hMax, sp, false) * 1.1);
  const xOf = (h) => inner.x + (h / hMax) * inner.w;
  const yOf = (m) => inner.y + inner.h - (m / dMax) * inner.h;

  // Outcome bands: below the inner radius is a centered catch, inner..gate is an
  // edge catch, above the gate the object is missed. The drift climbs through
  // all three as uptime grows.
  ctx.fillStyle = 'rgba(103,217,140,0.07)'; ctx.fillRect(inner.x, yOf(CENTER_M), inner.w, inner.y + inner.h - yOf(CENTER_M));
  ctx.fillStyle = 'rgba(255,209,102,0.07)'; ctx.fillRect(inner.x, yOf(GATE_M), inner.w, yOf(CENTER_M) - yOf(GATE_M));
  ...
  ctx.textAlign = 'left'; ctx.textBaseline = 'bottom'; ctx.fillText('catch radius (object missed above)', inner.x + 4, yOf(GATE_M) - 2);
  // centered-catch radius line.
  ctx.save(); ctx.setLineDash([3, 4]); ctx.strokeStyle = col.ok; ctx.lineWidth = 1.2;
  ctx.beginPath(); ctx.moveTo(inner.x, yOf(CENTER_M)); ctx.lineTo(inner.x + inner.w, yOf(CENTER_M)); ctx.stroke(); ctx.restore();
  ctx.fillStyle = col.ok; ctx.textAlign = 'left'; ctx.textBaseline = 'bottom'; ctx.fillText('centered-catch radius', inner.x + 4, yOf(CENTER_M) - 2);
  ...
  for (let i = 0; i <= 100; i++) {
    const h = i / 100 * hMax;
    const m = gateDisplacementMeters(h, sp, patched());
    const X = xOf(h), Y = yOf(m);
    if (i) ctx.lineTo(X, Y); else ctx.moveTo(X, Y);
  }
  ctx.stroke();

  // 100 h reference marker (the classic ~0.34 s drift).
  const dh = gateDisplacementMeters(100, sp, false);
  ctx.fillStyle = col.mover;
  ...
  ctx.fillText('100 h', xOf(100) - 4, yOf(dh) - 4);
...
function tick(now) {
  ...
  last = now;
  if (running) {
    p += dt / 4.0;                 // ~4 s per pass, looping
    if (p >= 1) p = 0;
    // Age the system clock continuously: the whole point is watching the drift
    // creep up across passes until the catch fails, then it wraps and
...
window.playground.getState = function () {
  return {
    fields: [
      ...
      { key: 'clock', label: 'clock error (s)', value: clockError(hours(), patched()), format: 'float' },
      { key: 'offset', label: 'miss distance (m)', value: disp(), format: 'float' },
      { key: 'conf', label: 'catch confidence', value: confidence(), format: 'float' },
    ],
  };
};
...
window.playground.getInvariants = function () {
  try {
    ...
    const a = gateDisplacementMeters(20, speed(), false);
    const b = gateDisplacementMeters(40, speed(), false);
    const ratio = a > 0 ? b / a : 2;
    const err = Math.abs(ratio - 2);
    return [{
      key: 'linear',
      label: 'clock drift linear in uptime',
      value: `${(ERR_PER_TICK_S * 1e9).toFixed(1)} ns/tick`,
