...
window.playground.getState = function () {
  return {
    fields: [
      { key: 'electric-field', label: 'E field', value: state.E, format: 'float' },
      { key: 'magnetic-field', label: 'B field', value: state.B, format: 'float' },
      { key: 'charge', label: 'q', value: state.q, format: 'float' },
      { key: 'mass', label: 'm', value: state.m, format: 'float' }
    ]
  };
};
window.playground.getInvariants = function () {
  const omega_c = state.q * state.B / state.m;
  const status = omega_c > 0 ? 'pass' : 'drift';
  return [
    { key: 'cyclotron', label: 'Cyclotron freq', value: omega_c.toFixed(3), status }
  ];
};
