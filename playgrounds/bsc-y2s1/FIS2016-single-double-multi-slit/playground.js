...
window.playground.getState = function () {
  const d = A_DEF * state.ratio;
  return {
    fields: [
      { key: 'num-slits', label: 'Number of slits', value: state.N, format: undefined },
      { key: 'separation', label: 'Separation (um)', value: d, format: 'float' },
      { key: 'ratio', label: 'd / a ratio', value: state.ratio, format: 'float' },
      { key: 'wavelength', label: 'Wavelength (um)', value: LAMBDA, format: 'float' }
    ]
  };
};
window.playground.getInvariants = function () {
  const d = A_DEF * state.ratio;
  const maxima = principalMaxima(d, LAMBDA, 3);
  let peakTest = 'ok';
  if (maxima.length > 0) {
    const I_max = intensity(maxima[0], state.N, A_DEF, d, LAMBDA);
    const expected = state.N * state.N;
    const err = Math.abs(I_max - expected) / expected;
    peakTest = err.toExponential(2);
  }
  const status = maxima.length > 0 && parseFloat(peakTest) < 1.0 ? 'pass' : 'drift';
  return [
    { key: 'principal-peak', label: 'Principal max vs N^2', value: peakTest, status }
  ];
};
