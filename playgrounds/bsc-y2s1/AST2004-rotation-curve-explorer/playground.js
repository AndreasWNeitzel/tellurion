...
window.playground.getState = function () {
  ...
  const vAt20 = vModel(20, model);
  const chi2 = chiSquared(model, state.data);
  return {
    fields: [
      { key: 'model', label: 'Model', value: model, format: undefined },
      { key: 'time', label: 'Time (Gyr)', value: state.t, format: 'float' },
      { key: 'v-at-20kpc', label: 'v(20 kpc)', value: vAt20, format: 'float' },
      { key: 'chi-sq', label: 'Chi^2', value: chi2, format: 'float' }
    ]
  };
};
window.playground.getInvariants = function () {
  const chi2 = chiSquared(state.model, state.data);
  const status = chi2 < 1000 ? 'pass' : 'pending';
  return [
    { key: 'model-fit', label: 'Chi-squared', value: chi2.toFixed(1), status }
  ];
};
