...
window.playground.getState = function () {
  return {
    fields: [
      { key: 'preset', label: 'Preset', value: st.preset, format: undefined },
      { key: 'wavelength', label: 'Wavelength (cells)', value: st.lambda, format: 'float' },
      { key: 'steps', label: 'Time steps', value: st.nstep, format: undefined },
      { key: 'damping', label: 'Damping gamma', value: st.gamma, format: 'float' }
    ]
  };
};
window.playground.getInvariants = function () {
  return [
    { key: 'wave-sim', label: 'Wave simulation running', value: st.nstep > 0 ? 'yes' : 'no', status: 'pass' }
  ];
};
