...
// State reports the Lame parameters and the P- and S-wave speeds.
// The invariants are the two facts an elastic-wave solver must
// respect: the P wave is always faster than the S wave, and the
// FDTD timestep stays inside the CFL stability limit.
window.playground = window.playground || {};
window.playground.getState = function () {
  const { vP, vS } = speeds(st.lambda, st.mu, RHO);
  return {
    fields: [
      { key: 'lambda', label: 'Lame lambda', value: st.lambda, format: 'float' },
      { key: 'mu', label: 'shear modulus mu', value: st.mu, format: 'float' },
      { key: 'vp', label: 'P-wave speed', value: vP, format: 'float' },
      { key: 'vs', label: 'S-wave speed', value: vS, format: 'float' },
    ],
  };
};
window.playground.getInvariants = function () {
  const { vP, vS } = speeds(st.lambda, st.mu, RHO);
  const cfl = DT * vP * Math.SQRT2;                 // dx = 1
  return [
    {
      key: 'wave-speeds',
      label: 'P-wave faster than S-wave (vP > vS)',
      value: `vP/vS = ${(vP / vS).toFixed(3)}`,
      status: vP > vS ? 'pass' : 'drift',
    },
    {
      key: 'cfl',
      label: 'FDTD timestep within the CFL stability limit',
      value: cfl.toFixed(3),
      status: cfl <= 1 ? 'pass' : (cfl <= 1.05 ? 'pending' : 'drift'),
    },
  ];
};
