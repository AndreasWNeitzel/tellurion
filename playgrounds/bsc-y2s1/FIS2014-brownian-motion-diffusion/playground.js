...
window.playground.getState = function () {
  const D = stokesEinstein(st.T, st.eta, st.r);
  const m = msd(ens);
  return {
    fields: [
      { key: 'temperature', label: 'Temperature (K)', value: st.T, format: 'float' },
      { key: 'viscosity', label: 'Viscosity (mPa s)', value: st.eta, format: 'float' },
      { key: 'radius', label: 'Radius (nm)', value: st.r, format: 'float' },
      { key: 'diffusion', label: 'D (m^2/s)', value: D, format: 'float' }
    ]
  };
};
window.playground.getInvariants = function () {
  const D = stokesEinstein(st.T, st.eta, st.r);
  const status = D > 0 ? 'pass' : 'drift';
  return [
    { key: 'stokes-einstein', label: 'Diffusion D > 0', value: D.toExponential(2) + ' m^2/s', status }
  ];
};
