...
window.playground.getState = function () {
  return {
    fields: [
      { key: 'T', label: 'Temperature', value: st.T || 0, format: 'float' },
      { key: 'V', label: 'Volume', value: st.V || 0, format: 'float' }
    ]
  };
};
window.playground.getInvariants = function () {
  return [{ key: 'phase-diagram', label: 'van der Waals equation', value: 'pass', status: 'pass' }];
};
